...
func (environment) OverviewCatalog(
	ctx context.Context,
	db storage.Executor,
	applicationID, environmentID uuid.UUID,
	includeRuntime bool,
) (EnvironmentOverviewRows, error) {
	...
	if includeRuntime {
		err = db.NewSelect().
			...
			Scan(ctx, &result.RuntimeServers)
		if err != nil {
			return result, err
		}
	}
	err = db.NewSelect().
		...
		ColumnExpr("connection.id, connection.alias, resource.name, resource.configuration ->> 'engine' AS engine").
		Join("JOIN resources AS resource ON resource.id = connection.resource_id").
		Where("connection.environment_id = ?", environmentID).
		Where("connection.archived_at IS NULL").
		OrderExpr("connection.alias").
		Scan(ctx, &result.Resources)
	if err != nil {
		return result, err
	}
	err = db.NewSelect().
		...
		Limit(20).
		Scan(ctx, &result.Builds)
	if err != nil {
		return result, err
	}
	err = db.NewSelect().
		TableExpr("releases").
		ColumnExpr("id, COALESCE(source_revision, version, '') AS source_revision, artifact_reference, created_at").
		Where("environment_id = ?", environmentID).
		Where("registry_resource_id IS NOT NULL").
		OrderExpr("created_at DESC").
		Limit(20).
		Scan(ctx, &result.Releases)
	if err != nil {
		return result, err
	}
	err = db.NewSelect().
		...
		Limit(30).
		Scan(ctx, &result.Deployments)
	if err != nil {
		return result, err
	}
	err = db.NewSelect().
		TableExpr("instances AS instance").
		ColumnExpr("instance.id, instance.state, instance.slot, instance.process_name, instance.process_kind, instance.replica_key, instance.ports, instance.release_id, instance.deployment_id, instance.environment_target_id AS target_id, server.name AS target_name, instance.observed_at").
		...
		OrderExpr("target.attached_at, instance.process_kind, instance.process_name, instance.replica_key, instance.created_at DESC").
		Limit(200).
		Scan(ctx, &result.Instances)
	if err != nil {
		return result, err
	}
	err = db.NewSelect().
		TableExpr("release_command_executions AS execution").
		ColumnExpr("execution.id, execution.status, execution.attempt, COALESCE(execution.external_id, '') AS external_id, execution.exit_code, execution.started_at, execution.finished_at, COALESCE(execution.error, '') AS error, execution.release_id, execution.environment_target_id AS target_id, server.name AS target_name, execution.configuration ->> 'command' AS command, execution.configuration -> 'arguments' AS arguments, (execution.configuration ->> 'timeout_seconds')::integer AS timeout_seconds, execution.created_at").
		Join("JOIN releases AS release ON release.id = execution.release_id").
		Join("JOIN environment_targets AS target ON target.id = execution.environment_target_id").
		Join("JOIN servers AS server ON server.id = target.server_id").
		Where("release.environment_id = ?", environmentID).
		OrderExpr("execution.created_at DESC").
		Limit(20).
		Scan(ctx, &result.ReleaseCommands)
	return result, err
}
...
func (instance) ServingForEnvironment(
	ctx context.Context,
	db storage.Executor,
	environmentID uuid.UUID,
) (EnvironmentServingInstance, error) {
...
func (environmentTargetState) ActiveForEnvironment(
	ctx context.Context,
	db storage.Executor,
	environmentID uuid.UUID,
	...
	err := db.NewSelect().
		...
		Where("target.environment_id = ?", environmentID).
		Where("target.detached_at IS NULL").
		OrderExpr("target.attached_at, target.id").
		Scan(ctx)
	return rows, err
}

func (deployment) LatestFailedRevisionID(
	ctx context.Context,
	db storage.Executor,
	targetID uuid.UUID,
) (*uuid.UUID, error) {
	...
	err := db.NewSelect().
		...
		Where("deployment.environment_target_id = ?", targetID).
		Where("deployment.status = 'failed'").
		OrderExpr("deployment.finished_at DESC NULLS LAST, deployment.created_at DESC").
		Limit(1).
		Scan(ctx, &id)
...
func (environmentResource) EditRows(
	ctx context.Context,
	db storage.Executor,
	environmentID uuid.UUID,
) ([]EnvironmentEditResourceRow, error) {
...
func (deployment) EnvironmentActivity(
	ctx context.Context,
	db storage.Executor,
	environmentID, deploymentID uuid.UUID,
) (EnvironmentDeploymentActivity, error) {
...
func (server) EnvironmentSetupOptions(
	ctx context.Context,
	db storage.Executor,
) ([]EnvironmentSetupServerOption, error) {
...
func (environmentSource) LatestActiveID(
	ctx context.Context,
	db storage.Executor,
	environmentID uuid.UUID,
) (*uuid.UUID, error) {
...
func (environment) ProductionForApplication(
	ctx context.Context,
	db storage.Executor,
...
func (environmentNetwork) ActivePrivateNetworkID(
	ctx context.Context,
	db storage.Executor,
	environmentID uuid.UUID,
) (uuid.UUID, error) {
	var id uuid.UUID
	err := db.NewSelect().
		TableExpr("environment_networks").
		ColumnExpr("private_network_id").
		Where("environment_id = ?", environmentID).
...
func (releaseCommandExecution) ActiveCountForEnvironment(
	ctx context.Context,
	db storage.Executor,
	environmentID uuid.UUID,
) (int, error) {
...
func (environment) EnsureSlugAvailableExcluding(
	ctx context.Context,
	db storage.Executor,
	applicationID, environmentID uuid.UUID,
	slug string,
...
func (environmentDomain) PrimaryForEnvironment(
	ctx context.Context,
	db storage.Executor,
	environmentID uuid.UUID,
...
func (environmentResource) ActiveForEnvironment(
	ctx context.Context,
	db storage.Executor,
	environmentID uuid.UUID,
...
func (resourceCredential) ArchiveID(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
	at time.Time,
...
func (environmentTargetState) MarkEnvironmentPending(
	ctx context.Context,
	db storage.Executor,
...
func (environmentTarget) ServerIDsForEnvironment(
	ctx context.Context,
	db storage.Executor,
	environmentID uuid.UUID,
) ([]uuid.UUID, error) {
...
func (change) RehomeDurableForEnvironments(
	ctx context.Context,
	db storage.Executor,
	...
	if err := db.NewSelect().
		TableExpr("environments AS environment").
...
func (application) LockNonSystem(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
...
func (environment) CountForApplication(
	ctx context.Context,
	db storage.Executor,
	applicationID uuid.UUID,
) (int, error) {
...
func (environment) LockIDsForApplication(
	ctx context.Context,
	db storage.Executor,
	applicationID uuid.UUID,
) ([]uuid.UUID, error) {
...
func (build) CacheServerIDs(
	...
	err := db.NewSelect().
		...
		Where("environment_id = ?", environmentID).
		Where("build_configuration ->> 'server_id' IS NOT NULL").
		Scan(ctx, &ids)
	return ids, err
}

func (buildpackConfiguration) ServerIDForEnvironment(
	ctx context.Context,
	db storage.Executor,
	environmentID uuid.UUID,
) (uuid.UUID, error) {
	var id uuid.UUID
	err := db.NewSelect().
		TableExpr("buildpack_configurations AS buildpack").
		ColumnExpr("buildpack.server_id").
		Join("JOIN environment_sources AS source ON source.id = buildpack.environment_source_id").
		Where("source.environment_id = ?", environmentID).
		Limit(1).
		Scan(ctx, &id)
	return id, err
}
...
func (application) SystemRuntimePlacement(
	ctx context.Context,
	db storage.Executor,
) (EnvironmentRuntimePlacement, error) {
...
func (serverNetwork) ActiveForServerNetwork(
	ctx context.Context,
	db storage.Executor,
...
func (resourceCredential) ActiveApplicationForEnvironment(
	ctx context.Context,
	db storage.Executor,
	resourceID, environmentID uuid.UUID,
...
func (environmentSource) SetupSource(
	ctx context.Context,
	db storage.Executor,
	applicationID, environmentID uuid.UUID,
) (EnvironmentSetupSource, error) {
