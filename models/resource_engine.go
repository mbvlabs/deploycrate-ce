...
var resourceEngineCatalog = []ResourceEngineDefinition{
	{
		...
		Protocols:        []string{"postgresql", "tcp"},
		EndpointRoles:    []string{"primary"},
		TLSModes:         resourceTLSModes(),
		CredentialFields: databaseCredentialFields(),
		EnvironmentKeys: resourceEnvironmentKeys(
			"POSTGRESQL",
			true,
			true,
			databaseCredentialFields(),
		),
		HealthCheckKinds: []string{"tcp", "postgresql"},
		DefaultPort:      5432,
		DefaultProtocol:  "postgresql",
	...
	},
	{
		...
		Label:            "MySQL",
		ResourceType:     ResourceTypeDatabase,
		Protocols:        []string{"mysql", "tcp"},
		EndpointRoles:    []string{"primary"},
		TLSModes:         resourceTLSModes(),
		CredentialFields: databaseCredentialFields(),
		EnvironmentKeys:  resourceEnvironmentKeys("MYSQL", true, false, databaseCredentialFields()),
		HealthCheckKinds: []string{"tcp", "mysql"},
		DefaultPort:      3306,
	...
	},
	{
		Engine:        "redis",
		Label:         "Redis",
		ResourceType:  ResourceTypeCache,
		Protocols:     []string{"redis", "tcp"},
		EndpointRoles: []string{"primary"},
		TLSModes:      resourceTLSModes(),
		CredentialFields: []ResourceCredentialField{
		...
		EnvironmentKeys: resourceEnvironmentKeys(
			...
			false,
			false,
			[]ResourceCredentialField{
				{Name: "password", Label: "Password", Required: false, Secret: true},
			},
		),
		HealthCheckKinds: []string{"tcp", "redis"},
		DefaultPort:      6379,
	...
	},
	{
		...
		Label:            "ClickHouse",
		ResourceType:     ResourceTypeDatabase,
		Protocols:        []string{"clickhouse", "http", "https", "tcp"},
		EndpointRoles:    []string{"primary", "wireguard"},
		TLSModes:         resourceTLSModes(),
		CredentialFields: databaseCredentialFields(),
		EnvironmentKeys: resourceEnvironmentKeys(
			"CLICKHOUSE",
		...
		HealthCheckKinds: []string{"tcp", "http", "clickhouse"},
		DefaultPort:      8123,
	...
	},
	{
		...
		HealthCheckKinds: []string{"http", "tcp"},
		DefaultPort:      5000,
		DefaultProtocol:  "http",
		DefaultTLSMode:   "disable",
	},
	{
		...
		ResourceType:  ResourceTypeService,
		Protocols:     []string{"http", "https"},
		EndpointRoles: []string{"local", "wireguard"},
		TLSModes:      resourceTLSModes(),
		CredentialFields: []ResourceCredentialField{
			{Name: "token", Label: "Environment identity token", Required: true, Secret: true},
		},
		EnvironmentKeys: []ResourceEnvironmentKeyDefinition{
			{Name: "endpoint", Label: "OTLP endpoint", DefaultKey: "OTEL_EXPORTER_OTLP_ENDPOINT"},
			{Name: "protocol", Label: "OTLP protocol", DefaultKey: "OTEL_EXPORTER_OTLP_PROTOCOL"},
			{Name: "headers", Label: "OTLP headers", DefaultKey: "OTEL_EXPORTER_OTLP_HEADERS"},
		},
		HealthCheckKinds: []string{"http", "tcp"},
		DefaultPort:      4318,
		DefaultProtocol:  "http",
	...
	},
	{
		...
		TLSModes:         resourceTLSModes(),
		CredentialFields: []ResourceCredentialField{},
		EnvironmentKeys:  resourceEnvironmentKeys("TCP", false, false, nil),
		HealthCheckKinds: []string{"tcp"},
		DefaultPort:      1,
		DefaultProtocol:  "tcp",
		DefaultTLSMode:   "disable",
	},
...
func resourceEnvironmentKeys(
	prefix string,
	database, connectionURL bool,
	credentialFields []ResourceCredentialField,
) []ResourceEnvironmentKeyDefinition {
	...
	for _, field := range credentialFields {
		definitions = append(
			...
			ResourceEnvironmentKeyDefinition{
				Name:       field.Name,
				Label:      field.Label,
				DefaultKey: prefix + "_" + NormalizeEnvironmentSecretKey(field.Name),
			},
		)
	}
	if connectionURL {
