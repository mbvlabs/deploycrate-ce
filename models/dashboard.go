...
// Dashboard is a read model: it deliberately has no backing table because it aggregates
// application, environment, deployment, Resource, and server persistence projections.
...
func (dashboard) Snapshot(ctx context.Context, exec storage.Executor) (DashboardSnapshot, error) {
	metrics := DashboardMetrics{}
	if err := exec.NewSelect().
		ColumnExpr("(SELECT COUNT(*) FROM applications WHERE archived_at IS NULL AND slug <> ?) AS applications", SystemApplicationSlug).
		ColumnExpr("(SELECT COUNT(*) FROM environments AS environment JOIN applications AS application ON application.id = environment.application_id WHERE environment.archived_at IS NULL AND application.archived_at IS NULL AND application.slug <> ?) AS environments", SystemApplicationSlug).
		ColumnExpr("(SELECT COUNT(*) FROM deployments AS deployment JOIN releases AS release ON release.id = deployment.release_id JOIN environments AS environment ON environment.id = release.environment_id JOIN applications AS application ON application.id = environment.application_id WHERE application.slug <> ?) AS deployments", SystemApplicationSlug).
		ColumnExpr("(SELECT COUNT(*) FROM deployments AS deployment JOIN releases AS release ON release.id = deployment.release_id JOIN environments AS environment ON environment.id = release.environment_id JOIN applications AS application ON application.id = environment.application_id WHERE application.slug <> ? AND deployment.status IN ('queued', 'running', 'cancelling', 'in_progress')) AS active_deployments", SystemApplicationSlug).
		ColumnExpr("(SELECT COUNT(*) FROM deployments AS deployment JOIN releases AS release ON release.id = deployment.release_id JOIN environments AS environment ON environment.id = release.environment_id JOIN applications AS application ON application.id = environment.application_id WHERE application.slug <> ? AND deployment.status = 'succeeded') AS successful_deployments", SystemApplicationSlug).
		ColumnExpr("(SELECT COUNT(*) FROM deployments AS deployment JOIN releases AS release ON release.id = deployment.release_id JOIN environments AS environment ON environment.id = release.environment_id JOIN applications AS application ON application.id = environment.application_id WHERE application.slug <> ? AND deployment.status IN ('succeeded', 'failed')) AS finished_deployments", SystemApplicationSlug).
		ColumnExpr("(SELECT COUNT(*) FROM resources WHERE archived_at IS NULL) AS resources").
		ColumnExpr("(SELECT COUNT(*) FROM servers WHERE archived_at IS NULL AND is_configured = TRUE) AS nodes").
	...
	if err := exec.NewSelect().TableExpr("deployments AS deployment").
	...
	if err := exec.NewSelect().TableExpr("applications AS application").
