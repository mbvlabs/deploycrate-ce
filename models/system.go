...
type SystemResourceHealthCheck struct {
	...
	Name             string          `json:"name" bun:"name"`
	Kind             string          `json:"kind" bun:"kind"`
	Configuration    json.RawMessage `json:"configuration" bun:"configuration"`
	IntervalSeconds  int32           `json:"intervalSeconds" bun:"interval_seconds"`
	TimeoutSeconds   int32           `json:"timeoutSeconds" bun:"timeout_seconds"`
	FailureThreshold int32           `json:"failureThreshold" bun:"failure_threshold"`
	SuccessThreshold int32           `json:"successThreshold" bun:"success_threshold"`
	Enabled          bool            `json:"enabled" bun:"enabled"`
}

type SystemDatabase struct {
	ResourceID                    string                      `json:"resourceId" bun:"resource_id"`
	ResourceCreatedAt             time.Time                   `json:"resourceCreatedAt" bun:"resource_created_at"`
	ResourceUpdatedAt             time.Time                   `json:"resourceUpdatedAt" bun:"resource_updated_at"`
	ResourceName                  string                      `json:"resourceName" bun:"resource_name"`
	Category                      string                      `json:"category" bun:"category"`
	Kind                          string                      `json:"kind" bun:"kind"`
	SharingScope                  string                      `json:"sharingScope" bun:"sharing_scope"`
	...
	EnvironmentName               string                      `json:"environmentName" bun:"environment_name"`
	EnvironmentKind               string                      `json:"environmentKind" bun:"environment_kind"`
	BindingID                     string                      `json:"bindingId" bun:"binding_id"`
	BindingCreatedAt              time.Time                   `json:"bindingCreatedAt" bun:"binding_created_at"`
	BindingUpdatedAt              time.Time                   `json:"bindingUpdatedAt" bun:"binding_updated_at"`
	BindingAlias                  string                      `json:"bindingAlias" bun:"binding_alias"`
	BindingConfiguration          json.RawMessage             `json:"bindingConfiguration" bun:"binding_configuration"`
	CredentialSource              string                      `json:"credentialSource" bun:"credential_source"`
	CredentialID                  string                      `json:"credentialId" bun:"credential_id"`
	CredentialName                string                      `json:"credentialName" bun:"credential_name"`
	CredentialRole                string                      `json:"credentialRole" bun:"credential_role"`
	CredentialUsername            string                      `json:"credentialUsername" bun:"credential_username"`
	CredentialMetadata            json.RawMessage             `json:"credentialMetadata" bun:"credential_metadata"`
	CredentialHasEncryptedPayload bool                        `json:"credentialHasEncryptedPayload" bun:"credential_has_encrypted_payload"`
	EndpointID                    string                      `json:"endpointId" bun:"endpoint_id"`
	EndpointCreatedAt             time.Time                   `json:"endpointCreatedAt" bun:"endpoint_created_at"`
	EndpointUpdatedAt             time.Time                   `json:"endpointUpdatedAt" bun:"endpoint_updated_at"`
	EndpointName                  string                      `json:"endpointName" bun:"endpoint_name"`
	EndpointRole                  string                      `json:"endpointRole" bun:"endpoint_role"`
	Address                       string                      `json:"address" bun:"address"`
	Port                          int32                       `json:"port" bun:"port"`
	...
	External                      bool                        `json:"external" bun:"external"`
	EndpointSettings              json.RawMessage             `json:"endpointSettings" bun:"endpoint_settings"`
	PrivateNetworkID              string                      `json:"privateNetworkId" bun:"private_network_id"`
	InstallationID                string                      `json:"installationId" bun:"installation_id"`
	InstallationCreatedAt         *time.Time                  `json:"installationCreatedAt" bun:"installation_created_at"`
	InstallationUpdatedAt         *time.Time                  `json:"installationUpdatedAt" bun:"installation_updated_at"`
	ImageReference                string                      `json:"imageReference" bun:"image_reference"`
	ImageDigest                   string                      `json:"imageDigest" bun:"image_digest"`
	ContainerName                 string                      `json:"containerName" bun:"container_name"`
	RestartPolicy                 string                      `json:"restartPolicy" bun:"restart_policy"`
	InstallationConfiguration     json.RawMessage             `json:"installationConfiguration" bun:"installation_configuration"`
	ServerID                      string                      `json:"serverId" bun:"server_id"`
	ServerName                    string                      `json:"serverName" bun:"server_name"`
	ServerAddress                 string                      `json:"serverAddress" bun:"server_address"`
	StatusRecorded                bool                        `json:"statusRecorded" bun:"status_recorded"`
	InstallationState             string                      `json:"installationState" bun:"installation_state"`
	InstalledVersion              string                      `json:"installedVersion" bun:"installed_version"`
	ServiceState                  string                      `json:"serviceState" bun:"service_state"`
	Health                        string                      `json:"health" bun:"health"`
	HealthReason                  string                      `json:"healthReason" bun:"health_reason"`
	StatusSource                  string                      `json:"statusSource" bun:"status_source"`
	StatusDetails                 json.RawMessage             `json:"statusDetails" bun:"status_details"`
	StatusObservedAt              *time.Time                  `json:"statusObservedAt" bun:"status_observed_at"`
	StatusExpiresAt               *time.Time                  `json:"statusExpiresAt" bun:"status_expires_at"`
	HealthChecks                  []SystemResourceHealthCheck `json:"healthChecks" bun:"-"`
}

func (a application) FindSystemDatabase(
	ctx context.Context,
	db storage.Executor,
	...
	if err := db.NewSelect().
		...
		ColumnExpr("resource.created_at AS resource_created_at").
		ColumnExpr("resource.updated_at AS resource_updated_at").
		ColumnExpr("resource.name AS resource_name").
		ColumnExpr("resource.category AS category").
		ColumnExpr("resource.kind AS kind").
		ColumnExpr("resource.sharing_scope AS sharing_scope").
		ColumnExpr("resource.owner_environment_id::text AS owner_environment_id").
		ColumnExpr("environment.id::text AS environment_id").
		ColumnExpr("environment.name AS environment_name").
		ColumnExpr("environment.kind AS environment_kind").
		ColumnExpr("binding.id::text AS binding_id").
		ColumnExpr("binding.created_at AS binding_created_at").
		ColumnExpr("binding.updated_at AS binding_updated_at").
		ColumnExpr("binding.alias AS binding_alias").
		ColumnExpr("binding.configuration AS binding_configuration").
		ColumnExpr("COALESCE(binding.configuration ->> 'credential_source', '') AS credential_source").
		ColumnExpr("COALESCE(credential.id::text, '') AS credential_id").
		ColumnExpr("COALESCE(credential.name, '') AS credential_name").
		ColumnExpr("COALESCE(credential.role, '') AS credential_role").
		ColumnExpr("COALESCE(credential.username, '') AS credential_username").
		ColumnExpr("COALESCE(credential.metadata, '{}'::jsonb) AS credential_metadata").
		ColumnExpr("COALESCE(octet_length(credential.enc_payload) > 0, FALSE) AS credential_has_encrypted_payload").
		ColumnExpr("endpoint.id::text AS endpoint_id").
		ColumnExpr("endpoint.created_at AS endpoint_created_at").
		ColumnExpr("endpoint.updated_at AS endpoint_updated_at").
		ColumnExpr("endpoint.name AS endpoint_name").
		ColumnExpr("endpoint.role AS endpoint_role").
		ColumnExpr("endpoint.address AS address").
		ColumnExpr("endpoint.port AS port").
		ColumnExpr("endpoint.protocol AS protocol").
		ColumnExpr("endpoint.tls_mode AS tls_mode").
		ColumnExpr("COALESCE(endpoint.settings ->> 'database', '') AS database_name").
		ColumnExpr("COALESCE((endpoint.settings ->> 'external')::boolean, FALSE) AS external").
		ColumnExpr("endpoint.settings AS endpoint_settings").
		ColumnExpr("COALESCE(endpoint.private_network_id::text, '') AS private_network_id").
		ColumnExpr("COALESCE(installation.id::text, '') AS installation_id").
		ColumnExpr("installation.created_at AS installation_created_at").
		ColumnExpr("installation.updated_at AS installation_updated_at").
		ColumnExpr("COALESCE(installation.image_reference, '') AS image_reference").
		ColumnExpr("COALESCE(installation.image_digest, '') AS image_digest").
		ColumnExpr("COALESCE(installation.container_name, '') AS container_name").
		ColumnExpr("COALESCE(installation.restart_policy, '') AS restart_policy").
		ColumnExpr("COALESCE(installation.configuration, '{}'::jsonb) AS installation_configuration").
		ColumnExpr("COALESCE(server.id::text, '') AS server_id").
		ColumnExpr("COALESCE(server.name, '') AS server_name").
		ColumnExpr("COALESCE(server.address, '') AS server_address").
		ColumnExpr("installation_status.resource_installation_id IS NOT NULL AS status_recorded").
		ColumnExpr("COALESCE(installation_status.state, '') AS installation_state").
		ColumnExpr("COALESCE(installation_status.installed_version, '') AS installed_version").
		ColumnExpr("COALESCE(installation_status.service_state, '') AS service_state").
		ColumnExpr("COALESCE(installation_status.health, '') AS health").
		ColumnExpr("COALESCE(installation_status.health_reason, '') AS health_reason").
		ColumnExpr("COALESCE(installation_status.source, '') AS status_source").
		ColumnExpr("COALESCE(installation_status.details, '{}'::jsonb) AS status_details").
		ColumnExpr("installation_status.observed_at AS status_observed_at").
		ColumnExpr("installation_status.expires_at AS status_expires_at").
		Join("JOIN environments AS environment ON environment.application_id = application.id AND environment.archived_at IS NULL").
		Join("JOIN environment_resources AS binding ON binding.environment_id = environment.id AND binding.alias = 'database' AND binding.archived_at IS NULL").
		Join("JOIN resources AS resource ON resource.id = binding.resource_id AND resource.category = 'database' AND resource.archived_at IS NULL").
		Join("JOIN resource_endpoints AS endpoint ON endpoint.id = binding.resource_endpoint_id AND endpoint.archived_at IS NULL").
		Join("LEFT JOIN resource_credentials AS credential ON credential.id = binding.resource_credential_id AND credential.archived_at IS NULL").
		Join("LEFT JOIN resource_installations AS installation ON installation.id = endpoint.resource_installation_id AND installation.archived_at IS NULL").
		...
		Limit(1).
		Scan(ctx, &database); err != nil {
		return SystemDatabase{}, err
	}

	database.HealthChecks = []SystemResourceHealthCheck{}
	if database.InstallationID == "" {
		return database, nil
	}
	if err := db.NewSelect().
		TableExpr("resource_health_checks AS health_check").
		ColumnExpr("health_check.id::text AS id").
		ColumnExpr("health_check.name AS name").
		ColumnExpr("health_check.kind AS kind").
		ColumnExpr("health_check.configuration AS configuration").
		ColumnExpr("health_check.interval_seconds AS interval_seconds").
		ColumnExpr("health_check.timeout_seconds AS timeout_seconds").
		ColumnExpr("health_check.failure_threshold AS failure_threshold").
		ColumnExpr("health_check.success_threshold AS success_threshold").
		ColumnExpr("health_check.enabled AS enabled").
		Where("health_check.resource_installation_id::text = ?", database.InstallationID).
		Where("health_check.archived_at IS NULL").
		OrderExpr("health_check.created_at ASC").
		Scan(ctx, &database.HealthChecks); err != nil {
		return SystemDatabase{}, err
	}
