...
func BuildReleaseCommandExecution(
	releaseID, revisionID, targetID, changeID uuid.UUID,
	opts ...ReleaseCommandExecutionOption,
) models.ReleaseCommandExecutionEntity {
	command := "/app/server"
	configuration, digest, _ := models.CanonicalReleaseCommandConfiguration(
	...
	f := &ReleaseCommandExecutionFactory{
		ReleaseCommandExecutionEntity: models.ReleaseCommandExecutionEntity{
			Status:                     "queued",
			Attempt:                    1,
			Configuration:              configuration,
			ConfigurationDigest:        digest,
			ReleaseID:                  releaseID,
			EnvironmentStateRevisionID: revisionID,
...
func CreateReleaseCommandExecution(
	ctx context.Context,
