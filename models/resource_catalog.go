...
func (resource) ListCatalog(
	ctx context.Context,
	db storage.Executor,
	filters ResourceListFilters,
) ([]ResourceListItem, error) {
...
func (resource) FormOptions(ctx context.Context, db storage.Executor) (ResourceFormOptions, error) {
	options := ResourceFormOptions{
		Servers:             make([]ResourceServerOption, 0),
		PrivateNetworks:     make([]ResourceNetworkOption, 0),
		RegistryCredentials: make([]ResourceRegistryCredentialOption, 0),
	}
	if err := db.NewSelect().
		TableExpr("servers AS server").
		ColumnExpr("server.id, server.name, server.address").
		Where("server.archived_at IS NULL").
		Where("server.is_configured = TRUE").
	...
	if err := db.NewSelect().
		...
		ColumnExpr("network.id, network.name").
		Where("network.archived_at IS NULL").
		OrderExpr("network.name").
		Scan(ctx, &options.PrivateNetworks); err != nil {
		return ResourceFormOptions{}, err
	}
	if err := db.NewSelect().
		TableExpr("credentials AS credential").
		ColumnExpr("credential.id, credential.name").
		Where("credential.archived_at IS NULL").
		OrderExpr("credential.name").
		Scan(ctx, &options.RegistryCredentials); err != nil {
		return ResourceFormOptions{}, err
	}
...
func (resource) DetailCatalog(
	...
	detail := ResourceDetails{
		Resource:      entity,
		Connections:   make([]ResourceConnectionDetail, 0),
		Endpoints:     make([]ResourceEndpointEntity, 0),
		Credentials:   make([]ResourceCredentialEntity, 0),
		Installations: make([]ResourceInstallationDetail, 0),
		Volumes:       make([]ResourceVolumeDetail, 0),
		Mounts:        make([]ResourceMountDetail, 0),
		HealthChecks:  make([]ResourceHealthCheckDetail, 0),
	}
	id := entity.ID
	if err := db.NewSelect().
	...
	if err := db.NewSelect().
		...
		Where("volume.resource_id = ?", id).
		Where("volume.archived_at IS NULL").
		OrderExpr("volume.name").
		Scan(ctx, &detail.Volumes); err != nil {
		return ResourceDetails{}, err
	}
	if err := db.NewSelect().
		...
		Scan(ctx, &detail.Mounts); err != nil {
		return ResourceDetails{}, err
	}
	if err := db.NewSelect().
		...
		Where("health_check.resource_id = ?", id).
		Where("health_check.archived_at IS NULL").
		OrderExpr("health_check.name").
		Scan(ctx, &detail.HealthChecks); err != nil {
		return ResourceDetails{}, err
	}
	return detail, nil
}
...
func (resource) ArchiveDependencies(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
) (ResourceArchiveDependencies, error) {
	var result ResourceArchiveDependencies
	var err error
	result.BindingCount, err = db.NewSelect().
	...
	result.PrivateAccessCount, err = db.NewSelect().
...
func (resource) ArchiveCascade(
	...
	statements := []struct{ table, where string }{
		{
...
func (resource) HasKindDependencies(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
) (bool, error) {
	count, err := db.NewSelect().
		TableExpr("resources AS resource").
		Where("resource.id = ?", id).
		Where(`EXISTS (SELECT 1 FROM resource_endpoints WHERE resource_id = resource.id AND archived_at IS NULL) OR EXISTS (SELECT 1 FROM resource_credentials WHERE resource_id = resource.id AND archived_at IS NULL) OR EXISTS (SELECT 1 FROM resource_health_checks WHERE resource_id = resource.id AND archived_at IS NULL)`).
		Count(ctx)
	return count > 0, err
}

func (resourceEndpoint) ActivePrimaryPublicCount(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
) (int, error) {
...
func (resourceEndpoint) ActiveForResource(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
...
func (serverNetwork) WireGuardAddress(
	ctx context.Context,
	db storage.Executor,
	serverID, networkID uuid.UUID,
) (string, error) {
	var address string
	err := db.NewSelect().
		TableExpr("server_networks").
...
func (resourceCredential) FindAdministrator(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
...
func (resourceEndpoint) FindPrimary(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
) (ResourceEndpointEntity, error) {
...
func (resourceCredential) ActiveForResourceAll(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
) ([]ResourceCredentialEntity, error) {
...
func (resource) PostgreSQLCredentialTopology(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
) (PostgreSQLCredentialTopology, error) {
...
func (privateNetwork) ActiveExists(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
) (bool, error) {
...
func (resourceEndpoint) WireGuardGatewayExists(
	ctx context.Context,
	db storage.Executor,
	resourceID, networkID uuid.UUID,
) (bool, error) {
	count, err := db.NewSelect().
		TableExpr("resource_endpoints").
		Where("resource_id = ?", resourceID).
...
func (environmentResource) IncompatibleEndpointNetworkCount(
	ctx context.Context,
	db storage.Executor,
	endpointID, networkID uuid.UUID,
) (int, error) {
	return db.NewSelect().
		TableExpr("environment_resources AS connection").
		Where("connection.resource_endpoint_id = ?", endpointID).
		Where("connection.archived_at IS NULL").
		Where("NOT EXISTS (SELECT 1 FROM environment_networks AS access WHERE access.environment_id = connection.environment_id AND access.private_network_id = ? AND access.removed_at IS NULL)", networkID).
		Count(ctx)
}

func (resourceEndpoint) LockActiveForResource(
	ctx context.Context,
	db storage.Executor,
...
func (resourceCredential) PostgreSQLCounts(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
	username string,
	excludeID *uuid.UUID,
) (PostgreSQLCredentialCounts, error) {
...
func (resourceCredential) LockActiveForResource(
	ctx context.Context,
	db storage.Executor,
...
func activeEnvironmentResourceConnections(
	ctx context.Context,
	db storage.Executor,
	column string,
	value uuid.UUID,
	...
	err := db.NewSelect().
		...
		Join("JOIN applications AS application ON application.id = environment.application_id AND application.archived_at IS NULL").
		Where(column+" = ?", value).
		Where("environment_resources.archived_at IS NULL").
		OrderExpr("environment_resources.environment_id, environment_resources.id").
		Scan(ctx)
	return rows, err
}

func (environmentResource) ActiveForResourceID(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
) ([]EnvironmentResourceEntity, error) {
...
func (environmentResource) ActiveForEndpointID(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
) ([]EnvironmentResourceEntity, error) {
...
func (environmentResource) ActiveForCredentialID(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
) ([]EnvironmentResourceEntity, error) {
...
func (environmentResource) LockActiveConnection(
	ctx context.Context,
	db storage.Executor,
...
func (resourceCredential) ActiveDependencyCount(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
) (int, error) {
...
func (resourceHealthCheck) ActiveKindCount(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
	kind string,
	...
	return db.NewSelect().
		TableExpr("resource_health_checks").
		Where("resource_id = ?", resourceID).
		Where("kind = ?", kind).
		Where("archived_at IS NULL").
		Count(ctx)
...
func (resourceInstallation) ActiveCountForResource(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
) (int, error) {
...
func (resourceInstallation) LockActiveForResource(
	ctx context.Context,
	db storage.Executor,
...
func (resourceInstallation) FindActiveForResourceID(
	ctx context.Context,
	db storage.Executor,
	resourceID, installationID uuid.UUID,
) (ResourceInstallationEntity, error) {
...
func (resourceInstallation) ActiveForResource(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
) ([]ResourceInstallationEntity, error) {
...
func (resourceInstallation) MoveConflicts(
	ctx context.Context,
	db storage.Executor,
	installationID, targetServerID uuid.UUID,
) (InstallationMoveConflicts, error) {
	var result InstallationMoveConflicts
	var err error
	result.UnreachableNetworks, err = db.NewSelect().
	...
	result.IncompatibleVolumes, err = db.NewSelect().
		TableExpr("resource_volume_mounts AS mount").
		...
		Where("mount.archived_at IS NULL").
		Where("volume.server_id <> ?", targetServerID).
		Count(ctx)
	return result, err
}

func (resourceInstallation) ActiveMountDependencyCount(
	ctx context.Context,
	db storage.Executor,
...
func (resourceInstallation) ArchiveID(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
	at time.Time,
...
func (resourceRestore) ActiveCountForResource(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
) (int, error) {
	return db.NewSelect().
		...
		Where("restore.status IN (?, ?, ?)", ResourceRestoreStatusPending, ResourceRestoreStatusSafetyBackup, ResourceRestoreStatusRestoring).
		Count(ctx)
}

func (resourceVolume) ActiveCountForResource(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
) (int, error) {
...
func (resourceVolume) LockActiveForResource(
	ctx context.Context,
	db storage.Executor,
...
func (resourceVolumeMount) ActiveCountForVolume(
	ctx context.Context,
	db storage.Executor,
	volumeID uuid.UUID,
) (int, error) {
...
func (resourceVolumeMount) ActiveCountForResource(
	ctx context.Context,
	db storage.Executor,
	resourceID uuid.UUID,
) (int, error) {
...
func (resourceVolumeMount) Topology(
	ctx context.Context,
	db storage.Executor,
	volumeID, installationID uuid.UUID,
) (ResourceMountTopology, error) {
	...
	err := db.NewSelect().
		...
		Join("JOIN resource_installations AS installation ON installation.id = ? AND installation.archived_at IS NULL", installationID).
...
func (resourceVolumeMount) LockActive(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
...
func (resourceVolume) OwnedByResource(
	ctx context.Context,
	db storage.Executor,
	volumeID, resourceID uuid.UUID,
) (bool, error) {
	count, err := db.NewSelect().
		TableExpr("resource_volumes").
		Where("id = ?", volumeID).
...
func (resourceVolumeMount) LockActiveForResource(
	ctx context.Context,
	db storage.Executor,
	...
	err := db.NewSelect().
		...
		Where("resource_volume_mounts.id = ?", mountID).
		Where("volume.resource_id = ?", resourceID).
		Where("resource_volume_mounts.archived_at IS NULL").
		For("UPDATE OF resource_volume_mounts").
		Scan(ctx)
	return row, err
}

func (resourceVolumeMount) ArchiveID(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
	at time.Time,
...
func (resourceEndpoint) ActiveBelongsToResource(
	ctx context.Context,
	db storage.Executor,
	endpointID, resourceID uuid.UUID,
) (bool, error) {
	count, err := db.NewSelect().
		TableExpr("resource_endpoints").
		Where("id = ?", endpointID).
...
func (resourceCredential) ActiveBelongsToResource(
	ctx context.Context,
	db storage.Executor,
	credentialID, resourceID uuid.UUID,
) (bool, error) {
	count, err := db.NewSelect().
		TableExpr("resource_credentials").
		Where("id = ?", credentialID).
...
func (resourceHealthCheck) LockActiveForResource(
	ctx context.Context,
	db storage.Executor,
	resourceID, checkID uuid.UUID,
) (ResourceHealthCheckEntity, error) {
	var row ResourceHealthCheckEntity
	err := db.NewSelect().
		Model(&row).
		Where("resource_health_checks.id = ?", checkID).
		Where("resource_health_checks.resource_id = ?", resourceID).
		Where("resource_health_checks.archived_at IS NULL").
		For("UPDATE OF resource_health_checks").
		Scan(ctx)
	return row, err
}

func (resourceHealthCheck) ArchiveID(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
	at time.Time,
