...
func (githubAppSetupAttempt) Create(
	ctx context.Context,
	db storage.Executor,
	data CreateGitHubAppSetupAttemptData,
) (GitHubAppSetupAttemptEntity, error) {
	now := time.Now().UTC()
	entity := GitHubAppSetupAttemptEntity{
		ID:          uuid.New(),
		CreatedAt:   now,
		UpdatedAt:   now,
...
func (githubAppSetupAttempt) LockUsable(
	ctx context.Context,
	db storage.Executor,
	digest []byte,
	purpose string,
...
func (githubAppSetupAttempt) MarkCompleted(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
) error {
	now := time.Now().UTC()
	_, err := db.NewUpdate().
		TableExpr("github_app_setup_attempts").
...
func (githubAppSetupAttempt) MarkFailed(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
	operationErr error,
...
func (e *GitHubAppEntity) Validate() error {
	b := validation.NewBuilder()
	if e.ID == uuid.Nil || e.CredentialID == uuid.Nil || e.InstanceID == uuid.Nil ||
		e.ExternalID <= 0 ||
		e.OwnerID <= 0 {
...
func (githubApp) Create(
	ctx context.Context,
	db storage.Executor,
	data CreateGitHubAppData,
) (GitHubAppEntity, error) {
	now := time.Now().UTC()
	entity := GitHubAppEntity{
		ID:           uuid.New(),
		CreatedAt:    now,
		UpdatedAt:    now,
...
func (githubApp) ActiveByInstance(
	ctx context.Context,
	db storage.Executor,
	instanceID uuid.UUID,
) (GitHubAppEntity, error) {
...
func (githubApp) ActiveDependencyCount(
	ctx context.Context,
	db storage.Executor,
	id uuid.UUID,
) (int, error) {
	installationCount, err := db.NewSelect().
		TableExpr("github_installations").
		Where("github_app_id = ?", id).
		Where("archived_at IS NULL").
		Count(ctx)
	if err != nil {
		return 0, err
...
func (e *GitHubInstallationEntity) Validate() error {
	b := validation.NewBuilder()
	if e.ID == uuid.Nil || e.GitHubAppID == uuid.Nil || e.ExternalID <= 0 || e.AccountID <= 0 ||
		strings.TrimSpace(e.AccountLogin) == "" {
		b.Add("id", "required", "GitHub installation identity is required")
	}
	b.OneOf("account_type", e.AccountType, "User", "Organization")
	b.OneOf(
		"repository_selection",
		e.RepositorySelection,
		GitHubRepositorySelectionAll,
		GitHubRepositorySelectionSelected,
	)
	if !validJSONObjects(e.Permissions) || !validJSONStringArray(e.Events) {
...
func (githubInstallation) Upsert(
	ctx context.Context,
	db storage.Executor,
	data UpsertGitHubInstallationData,
) (GitHubInstallationEntity, error) {
	now := time.Now().UTC()
	entity := GitHubInstallationEntity{
		ID:                  uuid.New(),
		CreatedAt:           now,
		UpdatedAt:           now,
	...
	if _, err := db.ExecContext(
		ctx,
		"SELECT pg_advisory_xact_lock(hashtextextended(?, 0))",
		"github-installation:"+strconv.FormatInt(data.ExternalID, 10),
	); err != nil {
		return entity, err
	}
...
func (githubInstallation) FindByExternal(
	ctx context.Context,
	db storage.Executor,
	externalID int64,
) (GitHubInstallationEntity, error) {
...
func (githubInstallation) ListForApp(
	ctx context.Context,
	db storage.Executor,
	appID uuid.UUID,
) ([]GitHubInstallationEntity, error) {
	...
	err := db.NewSelect().
		Model(&entities).
		Where("github_app_id = ?", appID).
		Where("archived_at IS NULL").
		OrderExpr("account_login ASC").
		Scan(ctx)
	return entities, err
}
...
func (githubInstallation) ActiveSourceCount(
	...
	return db.NewSelect().
		TableExpr("github_environment_sources AS binding").
		Join("JOIN github_repositories AS repository ON repository.id = binding.github_repository_id").
		Join("JOIN environment_sources AS source ON source.id = binding.environment_source_id").
		Where("repository.github_installation_id = ?", id).
...
func (githubRepository) Upsert(
	ctx context.Context,
	db storage.Executor,
	data UpsertGitHubRepositoryData,
) (GitHubRepositoryEntity, error) {
	now := time.Now().UTC()
	entity := GitHubRepositoryEntity{
		ID:                   uuid.New(),
		CreatedAt:            now,
		UpdatedAt:            now,
		...
		HTMLURL:              data.HTMLURL,
		LastSyncedAt:         data.LastSyncedAt.UTC(),
	}
	if entity.ExternalID <= 0 || entity.GitHubInstallationID == uuid.Nil ||
		strings.TrimSpace(entity.FullName) == "" {
		return entity, errors.Join(
			ErrDomainValidation,
			errors.New("GitHub repository identity is required"),
		)
	}
	if _, err := db.ExecContext(
		ctx,
		"SELECT pg_advisory_xact_lock(hashtextextended(?, 0))",
		"github-repository:"+strconv.FormatInt(data.ExternalID, 10),
	); err != nil {
		return entity, err
	}
...
func (githubRepository) MarkMissingRemoved(
	ctx context.Context,
	db storage.Executor,
...
func (githubRepository) FindByExternal(
	ctx context.Context,
	db storage.Executor,
	externalID int64,
) (GitHubRepositoryEntity, error) {
...
func (githubEnvironmentSource) Create(
	ctx context.Context,
	db storage.Executor,
	sourceID, repositoryID uuid.UUID,
) (GitHubEnvironmentSourceEntity, error) {
	now := time.Now().UTC()
	entity := GitHubEnvironmentSourceEntity{
		EnvironmentSourceID: sourceID,
...
func (githubEnvironmentSource) MatchingActive(
	ctx context.Context,
	db storage.Executor,
	installationExternalID, repositoryExternalID int64,
	reference string,
) ([]GitHubMatchingSource, error) {
	...
	err := db.NewSelect().TableExpr("github_environment_sources AS binding").
		...
		ColumnExpr("source.environment_id").
		ColumnExpr("source.reference").
		ColumnExpr("source.auto_build").ColumnExpr("repository.full_name AS repository_full_name").
		...
		Where("installation.external_id = ?", installationExternalID).
		Where("repository.external_id = ?", repositoryExternalID).
		...
		Where("repository.removed_at IS NULL").
		Where("installation.archived_at IS NULL").
		Where("installation.suspended_at IS NULL").
		Where("app.archived_at IS NULL").Scan(ctx, &entities)
	return entities, err
}
...
func (githubWebhookDelivery) CreateOrFind(
	ctx context.Context,
	db storage.Executor,
	data CreateGitHubWebhookDeliveryData,
) (GitHubWebhookDeliveryEntity, bool, error) {
	...
	entity := GitHubWebhookDeliveryEntity{
		...
		Event:                  data.Event,
		Action:                 data.Action,
		InstallationExternalID: data.InstallationExternalID,
		RepositoryExternalID:   data.RepositoryExternalID,
		BodyDigest:             data.BodyDigest,
		Payload:                data.Payload,
		ReceivedAt:             data.ReceivedAt,
		Status:                 GitHubDeliveryReceived,
	}
	_, err = db.NewInsert().Model(&entity).Exec(ctx)
...
func (githubWebhookDelivery) Mark(
	...
	now := time.Now().UTC()
	query := db.NewUpdate().
		TableExpr("github_webhook_deliveries").
		Set("status = ?", status).
		Set("updated_at = ?", now)
