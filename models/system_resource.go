...
func (application) FindSystemTelemetryContainers(
	...
	err := db.NewRaw(`
		SELECT resource.id::text AS resource_id, resource.name AS resource_name,
			resource.configuration ->> 'engine' AS resource_engine, installation.id::text AS installation_id,
			installation.container_name AS container_name
		...
		ORDER BY resource_name, installation_id`, serverID).Scan(ctx, &containers)
...
func (application) FindResourceAccessTarget(ctx context.Context, db storage.Executor, resourceID uuid.UUID) (ResourceAccessTarget, error) {
	...
	err := db.NewRaw(`
		SELECT resource.id AS resource_id, resource.name AS resource_name,
			resource.configuration ->> 'engine' AS resource_engine,
