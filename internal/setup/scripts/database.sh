...
: "${DB_INSTALLATION_ID:?DB_INSTALLATION_ID is required}"
...
if docker container inspect "${container}" >/dev/null 2>&1; then
  component="$(docker inspect --format '{{ index .Config.Labels "com.deploycrate.component" }}' "${container}")"
	installation_id="$(docker inspect --format '{{ index .Config.Labels "com.deploycrate.resource-installation" }}' "${container}")"
  logging_driver="$(docker inspect --format '{{.HostConfig.LogConfig.Type}}' "${container}")"
  if [ "${component}" != postgresql ] || [ "${installation_id}" != "${DB_INSTALLATION_ID}" ] || [ "${logging_driver}" != journald ]; then
...
if docker container inspect "${container}" >/dev/null 2>&1; then
  ...
  docker run --detach \
    ...
    --label com.deploycrate.component=postgresql \
		--label "com.deploycrate.resource-installation=${DB_INSTALLATION_ID}" \
