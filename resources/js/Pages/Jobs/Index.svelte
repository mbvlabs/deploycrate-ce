<script lang="ts">
  ...
  function pageHref(page: number) {
    ...
    return `${routes.systemJobs()}?${params.toString()}`
  }
  ...
  function applyFilters(event: SubmitEvent) {
    ...
    router.get(routes.systemJobs(), { search: search || undefined, state: state || undefined }, { replace: true })
  }
  ...
  <div class="space-y-8">
    <header class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div class="max-w-3xl">
        ...
        <h1 class="mt-3 text-3xl font-semibold tracking-tight">Jobs</h1>
        <p class="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">
          Inspect the River background queue, including scheduled work, active jobs, retries, and failures.
        </p>
      </div>
      ...
      <Card.Header>
        ...
        <Card.Description>
          {#if pagination.totalCount === 0}
            {hasFilters ? 'No jobs match these filters.' : 'No jobs have been enqueued yet.'}
          {:else}
            Showing {firstItem}-{lastItem} of {pagination.totalCount} matching jobs. Updates every four seconds.
          {/if}
        </Card.Description>
      </Card.Header>
      <Card.Content class="space-y-5">
        <form class="grid gap-3 sm:grid-cols-[minmax(0,1fr)_12rem_auto_auto]" onsubmit={applyFilters}>
          <Input name="search" value={filters.search} placeholder="Search by ID, kind, or queue" aria-label="Search jobs" />
          ...
          {#if hasFilters}<Button href={routes.systemJobs()} variant="ghost">Clear</Button>{/if}
        </form>

        <div class="overflow-x-auto border border-border">
          <table class="w-full min-w-[760px] text-left text-sm">
            <thead class="border-b border-border bg-muted/30 text-xs text-muted-foreground">
              <tr><th class="px-4 py-3 font-medium">Job</th><th class="px-4 py-3 font-medium">State</th><th class="px-4 py-3 font-medium">Queue</th><th class="px-4 py-3 font-medium">Attempts</th><th class="px-4 py-3 font-medium">Last activity</th></tr>
            </thead>
            <tbody class="divide-y divide-border">
              {#each items as item (item.id)}
                <tr class="hover:bg-muted/20">
                  <td class="px-4 py-3"><Link class="font-medium text-primary hover:underline" href={routes.systemJob(item.id)}>{item.kind}</Link><p class="mt-1 font-mono text-[11px] text-muted-foreground">#{item.id}</p></td>
              ...
              {:else}
                <tr><td colspan="5" class="px-4 py-10 text-center text-muted-foreground">No jobs to display.</td></tr>
              {/each}
            </tbody>
          </table>
        </div>
