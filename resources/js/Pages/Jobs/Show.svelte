...
<svelte:head><title>Job #{item.id}</title></svelte:head>

<DashboardLayout email={auth.email}>
  <div class="space-y-8">
    <header class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
      <div class="min-w-0">
        <p class="text-[10px] font-medium uppercase tracking-[0.24em] text-primary">System · Jobs</p>
        ...
        <p class="mt-3 font-mono text-xs text-muted-foreground">Job #{item.id}{isActive ? ' · live updates every three seconds' : ''}</p>
      </div>
      <div class="flex flex-wrap gap-2">
        <Button href={routes.systemJobs()} variant="outline">Back to jobs</Button>
        {#if item.canRun}<Button disabled={Boolean(activeAction)} onclick={() => askForAction({ action: 'run', url: routes.systemJobRun(item.id), title: 'Run job now?', description: 'Make this job immediately available to a background worker.', confirmLabel: 'Run job', destructive: false, method: 'post' })}>{activeAction === 'run' ? 'Starting...' : 'Run now'}</Button>{/if}
        {#if item.canRestart}<Button variant="outline" disabled={Boolean(activeAction)} onclick={() => askForAction({ action: 'restart', url: routes.systemJobRetry(item.id), title: 'Restart job?', description: 'Restart this job and retain its existing attempt history.', confirmLabel: 'Restart job', destructive: false, method: 'post' })}>{activeAction === 'restart' ? 'Restarting...' : 'Restart'}</Button>{/if}
        {#if item.canCancel}<Button variant="outline" disabled={Boolean(activeAction)} onclick={() => askForAction({ action: 'cancel', url: routes.systemJobCancel(item.id), title: 'Cancel job?', description: 'A running worker will receive a cancellation signal.', confirmLabel: 'Cancel job', destructive: true, method: 'post' })}>{activeAction === 'cancel' ? 'Cancelling...' : 'Cancel'}</Button>{/if}
        {#if item.canDelete}<Button variant="destructive" disabled={Boolean(activeAction)} onclick={() => askForAction({ action: 'delete', url: routes.systemJobDestroy(item.id), title: 'Permanently delete job?', description: 'This removes the job and all of its attempt history. This cannot be undone.', confirmLabel: 'Delete job', destructive: true, method: 'delete' })}>{activeAction === 'delete' ? 'Deleting...' : 'Delete'}</Button>{/if}
    ...
    <section class="grid gap-4 lg:grid-cols-2">
      <Card.Root>
        <Card.Header><Card.Title>Execution</Card.Title><Card.Description>How River processes this job.</Card.Description></Card.Header>
        <Card.Content>
    ...
    <section class="grid gap-4 xl:grid-cols-2">
      ...
      <Card.Root><Card.Header><Card.Title>Metadata</Card.Title><Card.Description>River metadata recorded for this job.</Card.Description></Card.Header><Card.Content><JsonCode value={item.metadata} /></Card.Content></Card.Root>
