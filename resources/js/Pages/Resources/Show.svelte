        ...
        {#if selectedBackups?.policy}
          ...
          <Card.Root>
            ...
            <Card.Content>
              ...
              {#if selectedBackups.history.length > 0}<div class="overflow-hidden border border-border"><Table.Root><Table.Header><Table.Row><Table.Head>Status</Table.Head><Table.Head>Trigger</Table.Head><Table.Head>Scheduled</Table.Head><Table.Head>Size</Table.Head><Table.Head class="w-24 text-right"><span class="sr-only">Actions</span></Table.Head></Table.Row></Table.Header><Table.Body>{#each selectedBackups.history as item (item.id)}<Table.Row><Table.Cell><div><StatusBadge status={item.status} />{#if item.error}<p class="mt-1 max-w-md text-xs text-destructive">{item.error}</p>{/if}</div></Table.Cell><Table.Cell class="capitalize">{item.triggerType}</Table.Cell><Table.Cell>{observedLabel(item.scheduledAt)}</Table.Cell><Table.Cell>{bytesLabel(item.sizeBytes)}</Table.Cell><Table.Cell class="text-right">{#if item.canRestore}<Button size="sm" variant="destructive" onclick={() => openRestoreDialog(item)}>Restore</Button>{/if}</Table.Cell></Table.Row>{/each}</Table.Body></Table.Root></div>{/if}
            </Card.Content>
          </Card.Root>
          ...
          <Card.Root>
            ...
            <Card.Content>
              ...
              {#if selectedBackups.restores.length > 0}<div class="overflow-hidden border border-border"><Table.Root><Table.Header><Table.Row><Table.Head>Status</Table.Head><Table.Head>Requested</Table.Head><Table.Head>Source backup</Table.Head><Table.Head>Safety backup</Table.Head></Table.Row></Table.Header><Table.Body>{#each selectedBackups.restores as restore (restore.id)}<Table.Row><Table.Cell><div><StatusBadge status={restore.status} />{#if restore.error}<p class="mt-1 max-w-md text-xs text-destructive">{restore.error}</p>{/if}</div></Table.Cell><Table.Cell>{observedLabel(restore.requestedAt)}</Table.Cell><Table.Cell>{observedLabel(restore.backupScheduledAt)}</Table.Cell><Table.Cell class="font-mono text-xs">{restore.safetyBackupId?.slice(0, 8) ?? 'None'}</Table.Cell></Table.Row>{/each}</Table.Body></Table.Root></div>{/if}
            </Card.Content>
          </Card.Root>
        {/if}
      </div>
          ...
          <Card.Content>
            ...
            {#if privateAccess}
              ...
              <div class="mt-6 space-y-3">
                ...
                {#if resource.deviceGrants.length > 0}<div class="overflow-hidden border border-border"><Table.Root><Table.Header><Table.Row><Table.Head>Device</Table.Head><Table.Head>Address</Table.Head><Table.Head>State</Table.Head><Table.Head>Latest handshake</Table.Head><Table.Head class="text-right">Actions</Table.Head></Table.Row></Table.Header><Table.Body>{#each resource.deviceGrants as grant}{@const grantState = grant.applicationState === 'applied' ? 'ready' : grant.applicationState === 'failed' ? 'failed' : 'applying'}<Table.Row><Table.Cell><div><span class="font-medium">{grant.deviceName}</span>{#if grant.applicationError}<p class="mt-1 max-w-xs text-xs text-destructive">{grant.applicationError}</p>{/if}</div></Table.Cell><Table.Cell class="font-mono text-xs">{grant.privateAddress}</Table.Cell><Table.Cell><StatusBadge status={grantState} label={accessStateLabel(grantState)} /></Table.Cell><Table.Cell>{observedLabel(grant.latestHandshakeAt)}</Table.Cell><Table.Cell><div class="flex justify-end gap-2">{#if grant.applicationState !== 'applied'}<Button size="sm" variant="ghost" onclick={() => retryDevice(grant.deviceId)}>Retry</Button>{/if}<Button size="sm" variant="destructive" onclick={() => revokeDevice(grant.deviceId, grant.deviceName)}>Revoke</Button></div></Table.Cell></Table.Row>{/each}</Table.Body></Table.Root></div>{/if}
              </div>
            {/if}
          </Card.Content>
        </Card.Root>
