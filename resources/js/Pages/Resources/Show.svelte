<script lang="ts">
  ...
  const definition = $derived(
    options.engines.find((kind) => kind.engine === resource.engine) ?? {
  ...
  const endpointNetworks = $derived(
    resource.installations.length === 1
      ? options.privateNetworks.filter(
          (network) =>
            network.serverIds.includes(resource.installations[0].serverId) &&
            Boolean(
              network.serverAddresses[resource.installations[0].serverId],
            ),
        )
  ...
  const privateNetwork = $derived(
    privateAccess
      ? options.privateNetworks.find(
          (item) => item.id === privateAccess.privateNetworkId,
        )
      : undefined,
  );
  ...
  const canAddApplicationUser = $derived(
    (!databaseBacked || resource.databases.length > 0) &&
      (!managedPostgreSQL || containerRunning),
  );
  const selectedBackups = $derived(
    backups.databases.find(
      (item) => item.databaseName === selectedBackupDatabaseName,
    ) ?? backups.databases[0],
  );
  const configuredBackups = $derived(
    backups.databases.filter((item) => Boolean(item.policy)),
  );
  const lastSuccessfulBackup = $derived(
    selectedBackups?.history.find((item) => item.status === "verified"),
  );
  ...
  const overallStatus = $derived.by(() => {
    if (databaseBacked) {
      if (
        ...
        return {
          ...
          tone: "bad",
          detail:
            "The published Database access check reached its failure threshold.",
        };
      if (
        resource.healthChecks.some(
        ...
        return {
          ...
          tone: "warn",
          detail:
            "The published Database access check is failing below its threshold.",
        };
      if (
        resource.healthChecks.some(
        ...
        return {
          ...
          tone: "good",
          detail:
            "The published endpoint and application credential can access the Database.",
        };
      return {
        label: "Access unknown",
        tone: "neutral",
        detail: "No fresh Database access observation is available.",
      };
    }
    if (resource.installations.length === 0)
      return {
        label: "Not deployed",
        tone: "neutral",
        detail: "No runtime installation is configured.",
      };
    if (
      resource.healthChecks.some(
        (item: any) => item.enabled && item.state === "unhealthy",
      ...
      return {
        label: "Unhealthy",
        tone: "bad",
        detail: "A Resource health check reached its failure threshold.",
      };
    if (
      resource.healthChecks.some(
        (item: any) => item.enabled && item.state === "degraded",
      ...
      return {
        ...
        tone: "warn",
        detail:
          "A Resource health check is failing below its failure threshold.",
      };
    if (
      resource.healthChecks.some(
    ...
    if (running === resource.installations.length)
      return {
        label: "Running",
        tone: "good",
        detail: "All installations are running.",
      };
    if (running > 0)
      return {
        label: "Partially running",
        tone: "warn",
        detail: `${running} of ${resource.installations.length} installations are running.`,
      };
    if (
    ...
    if (resource.installations.every((item: any) => item.state === "missing"))
      return {
        label: "Not deployed",
        tone: "neutral",
        detail: "No container has been created for this Resource.",
      };
    return {
      label: "Unknown",
      tone: "neutral",
      detail: "DeployCrate could not confirm the current runtime state.",
    };
  });
  ...
  onMount(() => {
    const restoreInterval = window.setInterval(() => {
      if (section === "backups" && activeRestore)
  ...
  $effect(() => {
    if (
      !enrollment?.clientConfiguration ||
      !enrollment.grantId ||
      enrollment.grantId === shownEnrollmentGrantId
    )
      return;
  ...
  $effect(() => {
    const available =
      section === "backups" ? configuredBackups : backups.databases;
    if (
      available.some((item) => item.databaseName === selectedBackupDatabaseName)
    )
      return;
    const first = available[0];
    selectedBackupDatabaseName = first?.databaseName ?? "";
    if (!credential.database)
      credential.database = resource.databases[0]?.name ?? "";
  ...
  function initialEndpoint() {
    return {
      name: "Primary",
      role: definition?.endpointRoles[0] ?? "primary",
      audience: "local_system",
      addressSource: "system_loopback",
      address: "127.0.0.1",
      port: definition?.defaultPort ?? 1,
      protocol: definition?.defaultProtocol ?? "tcp",
  ...
  function initialMount() {
    return {
      mountPath: "/data",
      readOnly: false,
      resourceVolumeId: resource.volumes[0]?.id ?? "",
      resourceInstallationId: resource.installations[0]?.id ?? "",
    };
  }
  function initialHealth() {
    return {
      name: "Readiness",
      kind:
        resource.engine === "postgresql" || resource.engine === "clickhouse"
          ? resource.engine
          : (definition?.healthCheckKinds?.[0] ?? "tcp"),
      configurationText: "{}",
      intervalSeconds: 30,
      timeoutSeconds: 5,
      failureThreshold: 3,
      successThreshold: 1,
      enabled: true,
      resourceEndpointId: primaryEndpoint?.id ?? "",
      resourceCredentialId:
        applicationCredentials[0]?.id ?? administratorCredentials[0]?.id ?? "",
    };
  }
  function initialBackupPolicy(
    detail: DatabaseBackups | undefined = selectedBackups,
  ) {
    return {
      schedule: detail?.policy?.schedule ?? "0 2 * * *",
      backupDestinationId:
        detail?.policy?.backupDestinationId ??
        backups.destinations[0]?.id ??
        "",
  ...
  function selectBackupDatabase(databaseName: string) {
    ...
    backupPolicy = initialBackupPolicy(
      backups.databases.find((item) => item.databaseName === databaseName),
    );
  }
  ...
  function json(value: string) {
    try {
      ...
      jsonError = "Configuration and metadata must contain valid JSON.";
  ...
  function actionURL(url: string) {
    return `${url}${url.includes("?") ? "&" : "?"}returnTo=${section}`;
  }
  function openWireGuardConfiguration() {
  ...
  function openEndpointDialog(item: any = null) {
    ...
    const originPrivateNetworkId =
      caddy?.origin_private_network_id ?? item?.privateNetworkId ?? "";
    endpoint = item
      ? {
          name: item.name,
          role: item.role,
          audience: originPrivateNetworkId
            ? "environment"
            : ["127.0.0.1", "::1", "localhost"].includes(originAddress)
              ? "local_system"
              : "custom",
          addressSource: originPrivateNetworkId
            ? "server_wireguard"
            : ["127.0.0.1", "::1", "localhost"].includes(originAddress)
              ? "system_loopback"
              : "manual",
          address: originAddress,
          port: caddy?.origin_port ?? item.port,
          protocol: caddy?.origin_protocol ?? item.protocol,
          tlsMode: caddy?.origin_tls_mode ?? item.tlsMode,
          privateNetworkId: originPrivateNetworkId,
  ...
  function saveEndpoint() {
    ...
    const settings = {
      ...
      address_source: endpoint.addressSource,
      ...(resource.engine === "opentelemetry"
        ? {
            exposure:
              endpoint.audience === "environment" ? "environment" : "system",
            transport: "http/protobuf",
            authentication:
              endpoint.audience === "environment" ? "signed_identity" : "none",
          }
        : {}),
    };
    const payload = {
    ...
    const options = {
      ...
      onError: () => (endpointDialogOpen = true),
      onFinish: () => (dialogAction = ""),
    };
    if (editingEndpointId)
      router.patch(
        actionURL(
          routes.resourceEndpointUpdate(resource.id, editingEndpointId),
        ),
        payload,
        options,
      );
      ...
      router.post(
        actionURL(routes.resourceEndpointCreate(resource.id)),
        payload,
        options,
      );
  }
  function chooseEndpointNetwork(networkId: string) {
    ...
    const selected = options.privateNetworks.find(
      (network) => network.id === networkId,
    );
    const serverId = resource.installations[0]?.serverId;
    endpoint.address =
      selected && serverId
        ? (selected.serverAddresses[serverId] ?? "127.0.0.1")
        : "127.0.0.1";
  }
  function chooseEndpointAudience(audience: string) {
    ...
    endpoint.role =
      ...
      definition.endpointRoles.includes("wireguard")
        ? "wireguard"
        : audience === "local_system" &&
            definition.endpointRoles.includes("local")
          ? "local"
    ...
    if (audience === "local_system") {
      ...
      chooseEndpointNetwork(
        endpoint.privateNetworkId || endpointNetworks[0]?.id || "",
      );
    } else {
  ...
  function openCredentialDialog(item: any = null) {
    ...
    credential = item
      ? {
          ...
          purpose: item.metadata?.purpose ?? "application",
  ...
  function saveCredential() {
    if ((!editingCredentialId && !canAddApplicationUser) || dialogAction)
      return;
    ...
    const options = {
      ...
      onError: () => (credentialDialogOpen = true),
      onFinish: () => (dialogAction = ""),
    };
    if (editingCredentialId)
      router.patch(
        actionURL(
          routes.resourceCredentialUpdate(resource.id, editingCredentialId),
        ),
        payload,
        options,
      );
      ...
      router.post(
        actionURL(routes.resourceCredentialCreate(resource.id)),
        payload,
        options,
      );
  }
  function enablePrivateAccess() {
    if (dialogAction) return;
    dialogAction = "private-access";
    router.post(
      actionURL(routes.resourcePrivateAccessCreate(resource.id)),
      { privateNetworkId: privateAccessNetworkId },
      {
        onSuccess: () => (privateAccessDialogOpen = false),
        onError: () => (privateAccessDialogOpen = true),
        onFinish: () => (dialogAction = ""),
      },
    );
  }
  function disablePrivateAccess() {
    confirmDestructive({
      ...
      description:
        "The private listener will be removed and every device grant for this Resource will be revoked. Archive endpoints using WireGuard before turning access off.",
  ...
  function submitDevice() {
    ...
    router.post(
      actionURL(routes.resourcePrivateAccessDeviceCreate(resource.id)),
      device,
      {
        onSuccess: () => (deviceDialogOpen = false),
        onFinish: () => (pendingAction = ""),
      },
    );
  }
  function revokeDevice(deviceId: string, deviceName: string) {
    confirmDestructive({
      ...
      description:
        "The device-specific firewall rule will be removed. The enrolled device remains available for other Resources.",
      confirmationLabel: "Revoke access",
    });
  }
  function retryDevice(deviceId: string) {
    router.post(
      actionURL(routes.resourcePrivateAccessDeviceCreate(resource.id)),
      { deviceId, name: "" },
    );
  ...
  function saveVolume() {
    if (dialogAction) return;
    submit(() => {
      const configuration = json(volume.configurationText);
      dialogAction = "volume";
      const options = {
        onSuccess: () => {
        ...
        onError: () => (volumeDialogOpen = true),
        onFinish: () => (dialogAction = ""),
      };
      if (editingVolumeId)
        router.patch(
          actionURL(routes.resourceVolumeUpdate(resource.id, editingVolumeId)),
          { ...volume, configuration },
          options,
        );
      else
        router.post(
          actionURL(routes.resourceVolumeCreate(resource.id)),
          { ...volume, configuration },
          options,
        );
    });
  ...
  function openMountDialog(item: any = null) {
    ...
    mount = item
      ? {
          ...
          resourceVolumeId: item.resourceVolumeId,
          resourceInstallationId: item.resourceInstallationId,
        }
      : initialMount();
  ...
  function saveMount() {
    ...
    const options = {
      ...
      onError: () => (mountDialogOpen = true),
      onFinish: () => (dialogAction = ""),
    };
    if (editingMountId)
      router.patch(
        actionURL(routes.resourceMountUpdate(resource.id, editingMountId)),
        mount,
        options,
      );
    else
      router.post(
        actionURL(routes.resourceMountCreate(resource.id)),
        mount,
        options,
      );
  }
  ...
  function saveHealth() {
    if (dialogAction) return;
    submit(() => {
      const configuration = json(health.configurationText);
      dialogAction = "health";
      const options = {
        onSuccess: () => {
        ...
        onError: () => (healthDialogOpen = true),
        onFinish: () => (dialogAction = ""),
      };
      if (editingHealthId)
        router.patch(
          actionURL(
            routes.resourceHealthCheckUpdate(resource.id, editingHealthId),
          ),
        ...
        router.post(
          actionURL(routes.resourceHealthCheckCreate(resource.id)),
          { ...health, configuration },
          options,
        );
    });
  ...
  function resourceDefaultEnvironmentKey(key: EnvironmentKey) {
    return (
      resource.configuration?.environment_keys?.[key.name] ?? key.defaultKey
    );
  }
    ...
    router.patch(
      actionURL(
        routes.resourceConnectionEnvironmentKeysUpdate(
          resource.id,
          connectionKeysConnection.id,
        ),
      ),
      { environmentKeys: connectionKeys },
      {
  ...
  function databaseRouteName(name: string) {
  ...
  function saveBackupPolicy() {
    ...
    const options = {
      onSuccess: () => (backupPolicyDialogOpen = false),
      onError: () => (backupPolicyDialogOpen = true),
    };
    if (selectedBackups.policy)
      router.patch(
        actionURL(
          routes.resourceBackupPolicyUpdate(
            resource.id,
            databaseName,
            selectedBackups.policy.id,
          ),
        ),
        backupPolicy,
        options,
      ...
      router.post(
        actionURL(routes.resourceBackupPolicyCreate(resource.id, databaseName)),
        backupPolicy,
        options,
      );
  }
  function pauseBackupPolicy() {
    if (selectedBackups?.policy)
      router.post(
        actionURL(
          routes.resourceBackupPolicyPause(
            resource.id,
            databaseRouteName(selectedBackups.databaseName),
            selectedBackups.policy.id,
          ),
        ),
        {},
  ...
  function resumeBackupPolicy() {
    if (selectedBackups?.policy)
      router.post(
        actionURL(
          routes.resourceBackupPolicyResume(
            resource.id,
            databaseRouteName(selectedBackups.databaseName),
            selectedBackups.policy.id,
          ),
  ...
  function runBackupPolicy() {
    if (selectedBackups?.policy)
      router.post(
        actionURL(
          routes.resourceBackupPolicyRun(
            resource.id,
            databaseRouteName(selectedBackups.databaseName),
            selectedBackups.policy.id,
          ),
  ...
  function confirmBackupPolicyArchive() {
    ...
    confirmDestructive({
      kind: "archive-backup-policy",
      databaseName: selectedBackups.databaseName,
      policyId: selectedBackups.policy.id,
      title: "Archive backup policy?",
      description:
        "Future schedules stop immediately. Existing backup history and artifacts are retained.",
      confirmationLabel: "Archive policy",
    });
  }
  ...
  function submitRestore() {
    if (!restoreBackup || restoreConfirmation !== resource.name || dialogAction)
      return;
    if (!selectedBackups) return;
    dialogAction = "restore";
    router.post(
      actionURL(
        routes.resourceRestoreCreate(
          resource.id,
          databaseRouteName(selectedBackups.databaseName),
        ),
      ),
  ...
  function lifecycle(
    ...
    const route =
      action === "start"
        ? routes.resourceInstallationStart(resource.id, installationId)
        : action === "stop"
          ? routes.resourceInstallationStop(resource.id, installationId)
          : action === "restart"
            ? routes.resourceInstallationRestart(resource.id, installationId)
  ...
  function confirmContainerRemoval(
    ...
    confirmDestructive({
      ...
      description:
        "The Docker container will be removed. Its installation configuration and volumes will remain.",
  ...
  function confirmResourceArchive() {
    confirmDestructive({
      ...
      description:
        "The Resource, its Docker container, and its associated Docker volume will be permanently removed. Existing dependencies must be removed first, and volume data cannot be recovered.",
  ...
  function archiveEndpoint() {
    ...
    confirmDestructive({
      ...
      title: `Archive ${endpoint.name}?`,
      description:
        "The endpoint can no longer be selected by Environments or health checks.",
      confirmationLabel: "Archive endpoint",
    });
  }
  function archiveCredential() {
    if (!editingCredentialId) return;
    credentialDialogOpen = false;
    confirmDestructive({
      kind: "archive-credential",
      credentialId: editingCredentialId,
      title: `Archive ${credential.name}?`,
      description:
        "The credential can no longer be selected by Environments or health checks.",
  ...
  function archiveVolume() {
    ...
    confirmDestructive({
      ...
      title: `Archive ${volume.name}?`,
      description: "Remove its mounts before archiving this volume.",
      confirmationLabel: "Archive volume",
    });
  }
  function archiveMount() {
    ...
    confirmDestructive({
      kind: "archive-mount",
      mountId: editingMountId,
      title: `Archive mount ${mount.mountPath}?`,
      description:
        "The volume will no longer be mounted into this installation.",
      confirmationLabel: "Archive mount",
    });
  }
  function archiveHealth() {
    ...
    confirmDestructive({
      ...
      title: `Archive ${health.name}?`,
      description: "DeployCrate will stop evaluating this health check.",
      confirmationLabel: "Archive health check",
    });
  }
  ...
  function destructiveActionURL(action: DestructiveAction) {
    switch (action.kind) {
      case "remove-container":
        return routes.resourceInstallationRemove(
          resource.id,
          action.installationId,
        );
      case "disable-private-access":
        return routes.resourcePrivateAccessDestroy(resource.id);
      case "revoke-device":
        return routes.resourcePrivateAccessDeviceDestroy(
          resource.id,
          action.deviceId,
        );
      case "archive-endpoint":
        return routes.resourceEndpointDestroy(resource.id, action.endpointId);
      case "archive-credential":
      ...
      case "archive-mount":
        return routes.resourceMountDestroy(resource.id, action.mountId);
      case "archive-health":
        return routes.resourceHealthCheckDestroy(resource.id, action.healthId);
      case "archive-resource":
        return routes.resourceDestroy(resource.id);
      case "archive-backup-policy":
        return routes.resourceBackupPolicyDestroy(
          resource.id,
  ...
  async function copyWireGuardConfiguration() {
    ...
    try {
      ...
      toast.success("WireGuard configuration copied");
    } catch {
      toast.error("WireGuard configuration could not be copied");
    }
  }
...
<DashboardLayout email={auth.email} resourceNavigation={resource}>
  <div class="mx-auto max-w-6xl space-y-8">
    <header class="flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        >
          {resource.engine} · {resource.resourceType}
        </p>
        <h1 class="mt-3 text-3xl font-semibold">{resource.name}</h1>
        <p class="mt-2 text-sm text-muted-foreground">{sectionTitle}</p>
        {#if databaseBacked}<p class="mt-1 text-xs text-muted-foreground">
            {resource.databases.length}
            {resource.databases.length === 1 ? "Database" : "Databases"}
          </p>{/if}
      </div>
    </header>

    {#if Object.keys(errors).length > 0}<Alert.Root variant="destructive"
        ><Alert.Title>The item could not be created</Alert.Title
        ><Alert.Description
          >Review the dialog fields and try again.</Alert.Description
        ></Alert.Root
      >{/if}
    {#if jsonError}<Alert.Root variant="destructive"
        ><Alert.Title>Invalid configuration</Alert.Title><Alert.Description
          >{jsonError}</Alert.Description
        ></Alert.Root
      >{/if}
        ...
        <Card.Content class="grid gap-0 p-0 lg:grid-cols-[minmax(0,1fr)_18rem]">
          <div class="p-6">
            <div class="flex flex-wrap items-center gap-3">
              ...
              {#if resource.installations.length > 0}<StatusBadge
                  status="Docker"
                />{/if}
            </div>
            <h2 class="mt-5 text-xl font-semibold">
              {databaseBacked ? "Access status" : "Container status"}
            </h2>
          ...
          <div
            class="grid grid-cols-2 gap-5 border-t border-border bg-muted/20 p-6 text-sm lg:grid-cols-1 lg:border-l lg:border-t-0"
          >
            <DataField
              label="Installations"
              value={String(resource.installations.length)}
            />
            <DataField
              label="Attached Environments"
              value={String(resource.connectionCount)}
            />
          </div>
        </Card.Content>
      </Card.Root>
    ...
    {#if section === "databases" && databaseBacked}
      <Card.Root>
        <Card.Header>
          ...
          <Card.Description
            >Logical Databases published through this Resource's endpoints.</Card.Description
          >
        </Card.Header>
        <Card.Content>
          {#if resource.databases.length === 0}<p
              class="text-sm text-muted-foreground"
            >
              No Databases published.
            </p>{:else}
      ...
      <div class="space-y-6">
        <Card.Root>
          <Card.Header>
            ...
            <Card.Description
              >Manage encrypted logical backups by Database.</Card.Description
            >
          </Card.Header>
          <Card.Content>
            {#if configuredBackups.length === 0}
              <div class="flex flex-wrap items-center justify-between gap-3">
                <p class="text-sm text-muted-foreground">
                  No backup policy is configured. Set one up from Databases.
                </p>
                <Button size="sm" variant="outline"
                  >{#snippet child({ props })}<Link
                      {...props}
                      href={routes.resourceDatabases(resource.id)}
                      >Open Databases</Link
                    >{/snippet}</Button
                >
              </div>
            {:else if selectedBackups}
              <FormField label="Database">
                <NativeSelect.Root
                  value={selectedBackups.databaseName}
                  onchange={(event) =>
        ...
        {#if selectedBackups?.policy}
            ...
            <Card.Header>
              <Card.Action
                ><div class="flex items-center gap-2">
                  <StatusBadge
                    status={selectedBackups.policy.active ? "active" : "paused"}
                  /><Button
                    size="sm"
                    variant="outline"
                    onclick={() =>
                      openBackupPolicyDialog(selectedBackups.databaseName)}
                    >Edit policy</Button
                  >
                </div></Card.Action
              >
              ...
              <Card.Description
                >Schedule, verification status, and policy controls for {selectedBackups.databaseName}.</Card.Description
              >
            </Card.Header>
            <Card.Content>
              <div class="overflow-hidden border border-border">
                <Table.Root
                  ><Table.Header
                    ><Table.Row
                      ><Table.Head>Schedule</Table.Head><Table.Head
                        >Next run</Table.Head
                      ><Table.Head>Last successful</Table.Head><Table.Head
                        >Last verified</Table.Head
                  ...
                  ><Table.Body
                    ><Table.Row
                      ><Table.Cell class="font-mono text-xs"
                        >{selectedBackups.policy.schedule}</Table.Cell
                      ><Table.Cell
                        >{selectedBackups.policy.active
                          ? observedLabel(selectedBackups.policy.nextRunAt)
                          : "Paused"}</Table.Cell
                      ><Table.Cell
                        >{observedLabel(
                          lastSuccessfulBackup?.finishedAt ?? null,
                        )}</Table.Cell
                      ><Table.Cell
                        >{observedLabel(
                          lastSuccessfulBackup?.verifiedAt ?? null,
                        )}</Table.Cell
                      ><Table.Cell
                        ><div class="flex justify-end gap-2">
                          {#if selectedBackups.policy.active}<Button
                              size="sm"
                              variant="ghost"
                              onclick={pauseBackupPolicy}>Pause</Button
                            >{:else}<Button
                              size="sm"
                              variant="ghost"
                              onclick={resumeBackupPolicy}>Resume</Button
                            >{/if}<Button
                            size="sm"
                            variant="outline"
                            disabled={!selectedBackups.policy.active}
                            onclick={runBackupPolicy}>Run now</Button
                          ><Button
                            size="sm"
                            variant="destructive"
                            onclick={confirmBackupPolicyArchive}>Archive</Button
                          >
                        </div></Table.Cell
                      ></Table.Row
                    ></Table.Body
          ...
          <Card.Root>
            <Card.Header>
              ...
              <Card.Description
                >The latest backup attempts, including retained outcomes after
                policy archival.</Card.Description
              >
            </Card.Header>
            <Card.Content>
              {#if selectedBackups.history.length === 0}<p
                  class="text-sm text-muted-foreground"
                >
                  No backups have been requested for this Database.
                </p>{/if}
              {#if selectedBackups.history.length > 0}<div
                    ...
                    ><Table.Header
                      ><Table.Row
                        ><Table.Head>Status</Table.Head><Table.Head
                          >Trigger</Table.Head
                        ><Table.Head>Scheduled</Table.Head><Table.Head
                          >Size</Table.Head
                      ...
                      ></Table.Header
                    ><Table.Body
                      >{#each selectedBackups.history as item (item.id)}<Table.Row
                          ><Table.Cell
                            ><div>
                              <StatusBadge
                                status={item.status}
                              />{#if item.error}<p
                                  class="mt-1 max-w-md text-xs text-destructive"
                                >
                                  {item.error}
                                </p>{/if}
                            </div></Table.Cell
                          ><Table.Cell class="capitalize"
                            >{item.triggerType}</Table.Cell
                          ><Table.Cell
                            >{observedLabel(item.scheduledAt)}</Table.Cell
                          ><Table.Cell>{bytesLabel(item.sizeBytes)}</Table.Cell
                          ><Table.Cell class="text-right"
                            >{#if item.canRestore}<Button
                                size="sm"
                                variant="destructive"
          ...
          <Card.Root>
            <Card.Header>
              <Card.Action
                >{#if activeRestore}<span
                    class="border border-primary/50 px-2 py-1 text-xs text-primary"
                    >Restore in progress</span
                  >{/if}</Card.Action
              >
              <Card.Title>Restore history</Card.Title>
              <Card.Description
                >A safety backup is verified before any Database is changed.</Card.Description
              >
            </Card.Header>
            <Card.Content>
              {#if selectedBackups.restores.length === 0}<p
                  class="text-sm text-muted-foreground"
                >
                  No restores have been requested for this Database.
                </p>{/if}
              {#if selectedBackups.restores.length > 0}<div
                  class="overflow-hidden border border-border"
                >
                  <Table.Root
                    ><Table.Header
                      ><Table.Row
                        ><Table.Head>Status</Table.Head><Table.Head
                          >Requested</Table.Head
                        ><Table.Head>Source backup</Table.Head><Table.Head
                          >Safety backup</Table.Head
                        ></Table.Row
                      ></Table.Header
                    ><Table.Body
                      >{#each selectedBackups.restores as restore (restore.id)}<Table.Row
                          ><Table.Cell
                            ><div>
                              <StatusBadge
                          ...
                          ><Table.Cell
                            >{observedLabel(restore.requestedAt)}</Table.Cell
                          ><Table.Cell
                            >{observedLabel(
                              restore.backupScheduledAt,
                            )}</Table.Cell
                          ><Table.Cell class="font-mono text-xs"
                            >{restore.safetyBackupId?.slice(0, 8) ??
                              "None"}</Table.Cell
                          ></Table.Row
                        >{/each}</Table.Body
                    ></Table.Root
    ...
    {#if section === "endpoints"}
      <div class="space-y-6">
          ...
          <Card.Header
            ><Card.Action
              ><Button
                size="sm"
                variant="outline"
                onclick={() => openEndpointDialog()}>Add endpoint</Button
              ></Card.Action
            ><Card.Title>Endpoints</Card.Title><Card.Description
              >Every address published by this Resource, including endpoints
              routed through enabled WireGuard access.</Card.Description
            ></Card.Header
          >
          <Card.Content>
            {#if resource.endpoints.length === 0}<p
                class="text-sm text-muted-foreground"
              >
                  ...
                  <Table.Header
                    ><Table.Row
                      ><Table.Head>Name</Table.Head><Table.Head
                        >Address</Table.Head
                      ><Table.Head>Available to</Table.Head><Table.Head
                        >Caddy route</Table.Head
                  ...
                  <Table.Body>
                    {#each resource.endpoints as item (item.id)}
                      ...
                      <Table.Row>
                        ...
                        <Table.Cell class="font-mono text-xs"
                          >{item.address}:{item.port}</Table.Cell
                        >
                        <Table.Cell class="capitalize"
                          >{String(
                            item.settings?.audience ??
                              (item.privateNetworkId
                                ? "environment"
                                : "local system"),
                          ).replaceAll("_", " ")}</Table.Cell
                        >
                        <Table.Cell
                          >{#if publication}<div
                        ...
                        >
                        <Table.Cell class="text-right"
                          >{#if item.managed}<span
                              class="text-xs text-muted-foreground"
                              >Managed</span
                            >{:else}<Button
                              size="sm"
                              variant="ghost"
                              onclick={() => openEndpointDialog(item)}
                              >Edit</Button
                            >{/if}</Table.Cell
                        >
                      </Table.Row>
        ...
        <Card.Root>
          <Card.Header>
            <Card.Action
              >{#if privateAccess}<Button
                  size="sm"
                  variant="destructive"
                  onclick={disablePrivateAccess}
                  >Turn off WireGuard access</Button
                >{:else}<Button
                  size="sm"
                  variant="outline"
                  onclick={() => (privateAccessDialogOpen = true)}
                  >Turn on WireGuard access</Button
                >{/if}</Card.Action
            >
            <Card.Title>WireGuard network access</Card.Title>
            <Card.Description
              >Enable private access and manage the devices allowed to reach
              this Resource.</Card.Description
            >
          </Card.Header>
          <Card.Content>
            <div class="flex flex-wrap items-start justify-between gap-3">
              {#if !privateAccess}<p class="text-sm text-muted-foreground">
                  Turn on WireGuard access to create a managed private endpoint
                  and grant device access.
                </p>{/if}
            </div>
            {#if privateAccess}
              <div class="grid gap-5 sm:grid-cols-3">
                <DataField
                  label="Gateway"
                  value={`${privateAccess.address}:${privateAccess.port}`}
                />
                <DataField
                  label="Network"
                  value={privateNetwork?.name ?? "Unknown"}
                />
                <DataField
                  label="State"
                  value={accessStateLabel(resource.privateAccessState)}
                />
              </div>
              <div class="mt-6 space-y-3">
                <div class="flex flex-wrap items-center justify-between gap-3">
                  <h4 class="text-sm font-medium">Granted devices</h4>
                  <div class="flex items-center gap-2">
                    {#if enrollment?.clientConfiguration}<Button
                        size="sm"
                        variant="outline"
                        onclick={openWireGuardConfiguration}
                        >View client configuration</Button
                      >{/if}<Button
                      size="sm"
                      variant="outline"
                ...
                {#if resource.deviceGrants.length === 0}<p
                    class="text-sm text-muted-foreground"
                  >
                    No device grant has applied the listener yet. The Resource
                    is on the private network, but its listener is not ready.
                  </p>{/if}
                {#if resource.deviceGrants.length > 0}<div
                    class="overflow-hidden border border-border"
                  >
                    <Table.Root
                      ><Table.Header
                        ><Table.Row
                          ><Table.Head>Device</Table.Head><Table.Head
                            >Address</Table.Head
                          ><Table.Head>State</Table.Head><Table.Head
                            >Latest handshake</Table.Head
                          ><Table.Head class="text-right">Actions</Table.Head
                          ></Table.Row
                        ></Table.Header
                      ><Table.Body
                            ...
                            ><Table.Cell
                              ><div>
                                <span class="font-medium"
                                  >{grant.deviceName}</span
                                >{#if grant.applicationError}<p
                                    class="mt-1 max-w-xs text-xs text-destructive"
                                  >
                                    {grant.applicationError}
                                  </p>{/if}
                              </div></Table.Cell
                            ><Table.Cell class="font-mono text-xs"
                              >{grant.privateAddress}</Table.Cell
                            ><Table.Cell
                              ><StatusBadge
                                status={grantState}
                                label={accessStateLabel(grantState)}
                              /></Table.Cell
                            ><Table.Cell
                              >{observedLabel(
                                grant.latestHandshakeAt,
                              )}</Table.Cell
                            ><Table.Cell
                              ><div class="flex justify-end gap-2">
                                {#if grant.applicationState !== "applied"}<Button
                                    size="sm"
                                    variant="ghost"
                                    onclick={() => retryDevice(grant.deviceId)}
                                    >Retry</Button
                                  >{/if}<Button
                                  size="sm"
                                  variant="destructive"
            ...
            ><Card.Action
              ><div class="flex gap-2">
                <Button
                  size="sm"
                  variant="outline"
                  onclick={() => openVolumeDialog()}>Add volume</Button
                >{#if resource.volumes.length > 0 && resource.installations.length > 0}<Button
                    size="sm"
                    variant="outline"
                    onclick={() => openMountDialog()}>Add mount</Button
                  >{/if}
              </div></Card.Action
            ><Card.Title>Storage</Card.Title><Card.Description
              >Durable volumes and their installation mounts.</Card.Description
            ></Card.Header
          ><Card.Content class="space-y-5"
            >{#if resource.volumes.length === 0}<p
                class="text-sm text-muted-foreground"
              >
                ...
                <Table.Root
                  ><Table.Header
                    ><Table.Row
                      ><Table.Head>Volume</Table.Head><Table.Head
                        >Driver</Table.Head
                      ><Table.Head>Server</Table.Head><Table.Head class="w-20"
                        ><span class="sr-only">Actions</span></Table.Head
                      ></Table.Row
                    ></Table.Header
                  ><Table.Body
                    >{#each resource.volumes as item}<Table.Row
                        ...
                        ><Table.Cell class="text-right"
                          ><Button
                            size="sm"
                            variant="ghost"
                            onclick={() => openVolumeDialog(item)}>Edit</Button
                          ></Table.Cell
                        ></Table.Row
                      >{/each}</Table.Body
                  ></Table.Root
                >
              </div>{/if}{#if resource.mounts.length > 0}<div>
                ...
                <div class="overflow-hidden border border-border">
                  <Table.Root
                    ><Table.Header
                      ><Table.Row
                        ><Table.Head>Path</Table.Head><Table.Head
                          >Volume</Table.Head
                        ><Table.Head>Installation</Table.Head><Table.Head
                          >Mode</Table.Head
                        ><Table.Head class="w-20"
                          ><span class="sr-only">Actions</span></Table.Head
                        ></Table.Row
                    ...
                    ><Table.Body
                      >{#each resource.mounts as item}<Table.Row
                          ...
                          ><Table.Cell>{item.volumeName}</Table.Cell><Table.Cell
                            >{item.containerName}</Table.Cell
                          ><Table.Cell
                            >{item.readOnly
                              ? "Read only"
                              : "Read/write"}</Table.Cell
                          ><Table.Cell class="text-right"
                            ><Button
                              size="sm"
                              variant="ghost"
                              onclick={() => openMountDialog(item)}>Edit</Button
                            ></Table.Cell
                          ></Table.Row
                        >{/each}</Table.Body
                    ></Table.Root
        ...
        ><Card.Header
          ><Card.Action
            ><Button
              size="sm"
              variant="outline"
              disabled={resource.endpoints.length === 0 ||
                (databaseBacked && applicationCredentials.length === 0)}
              onclick={() => openHealthDialog()}>Add check</Button
            ></Card.Action
          ><Card.Title>Health checks</Card.Title><Card.Description
            >Desired checks and their latest observations.</Card.Description
          ></Card.Header
        ><Card.Content
          >{#if resource.healthChecks.length === 0}<p
              class="text-sm text-muted-foreground"
            >
              ...
              <Table.Root
                ><Table.Header
                  ><Table.Row
                    ><Table.Head>Name</Table.Head><Table.Head>Kind</Table.Head
                    ><Table.Head>State</Table.Head><Table.Head
                      >Interval</Table.Head
                    ><Table.Head>Observed</Table.Head><Table.Head class="w-20"
                      ><span class="sr-only">Actions</span></Table.Head
                    ></Table.Row
                  ></Table.Header
                ><Table.Body
                  >{#each resource.healthChecks as item}<Table.Row
                      ...
                      ><Table.Cell
                        ...
                        /></Table.Cell
                      ><Table.Cell>{item.intervalSeconds}s</Table.Cell
                      ><Table.Cell>{observedLabel(item.observedAt)}</Table.Cell
                      ><Table.Cell class="text-right"
                        ><Button
                          size="sm"
                          variant="ghost"
                          onclick={() => openHealthDialog(item)}>Edit</Button
                        ></Table.Cell
                      ></Table.Row
                    >{/each}</Table.Body
                ></Table.Root
    ...
    {#if section === "overview"}
        ...
        <Card.Header
          ><Card.Action
            ><span class="text-xs text-muted-foreground"
              >{resource.connectionCount} attached</span
            ></Card.Action
          ><Card.Title>Attached Environments</Card.Title><Card.Description
            >Attach Resources from Environment pages. Manage injected key names
            here on the Resource.</Card.Description
          ></Card.Header
        >
        <Card.Content class="space-y-3">
          {#if resource.connections.length === 0}<p
              class="text-sm text-muted-foreground"
            >
          ...
          {#each resource.connections as item}
            ...
            >
              <div>
                <p class="font-medium">
                  {item.applicationName} / {item.environmentName}
                </p>
                <p class="mt-1 text-xs text-muted-foreground">
                  Database {item.database || "Not specified"} · Alias {item.alias}
                  · Endpoint {item.endpointName}{item.credentialName
                    ? ` · Credential ${item.credentialName}`
                    : ""}
                </p>
                <p class="mt-2 font-mono text-xs text-muted-foreground">
                  {Object.values(
                    item.configuration.environment_keys ?? {},
                  ).join(" · ")}
                </p>
              </div>
              <div class="flex items-start gap-2">
                <span class="pt-2 text-xs text-muted-foreground"
                  >{Object.keys(item.environmentKeyOverrides ?? {}).length === 0
                    ? "Using defaults"
                    : `${Object.keys(item.environmentKeyOverrides ?? {}).length} overridden`}</span
                ><Button
                  size="sm"
                  variant="outline"
                  onclick={() => openConnectionKeys(item)}
                  >Edit injected keys</Button
                >
              </div>
            </div>
          {/each}
      ...
      <div class="border-t border-border pt-6">
        <Button variant="destructive" onclick={confirmResourceArchive}
          >Archive Resource</Button
        >
      </div>
    {/if}
  </div>
  ...
  <Dialog.Root bind:open={wireGuardConfigurationDialogOpen}>
    <Dialog.Content class="sm:max-w-3xl">
      <Dialog.Header>
        ...
        <Dialog.Description
          >Import this configuration on the newly enrolled device now. The
          private key is not stored and cannot be shown again after leaving this
          page.</Dialog.Description
        >
      </Dialog.Header>
      <pre
        class="max-h-[60vh] overflow-auto border border-border bg-muted/30 p-4 font-mono text-xs whitespace-pre-wrap">{enrollment?.clientConfiguration ??
          ""}</pre>
      <Dialog.Footer>
        <Button
          type="button"
          variant="outline"
          onclick={copyWireGuardConfiguration}>Copy configuration</Button
        >
        <Button
          type="button"
          onclick={() => (wireGuardConfigurationDialogOpen = false)}
  ...
  <ConfirmActionDialog
    ...
    requiredPhrase={destructiveAction?.kind === "archive-endpoint"
      ? "DELETE"
      : ""}
    ...
    <Dialog.Content>
      ...
      >
        <Dialog.Header>
          ...
          <Dialog.Description
            >DeployCrate first creates and verifies a fresh safety backup.
            Existing database sessions are terminated during the final cutover
            and clients must reconnect.</Dialog.Description
          >
        </Dialog.Header>
        {#if restoreBackup}<div
            class="border border-border bg-muted/20 p-3 text-sm"
          >
            <p>Backup from {observedLabel(restoreBackup.scheduledAt)}</p>
            <p class="mt-1 font-mono text-xs text-muted-foreground">
              {restoreBackup.id}
            </p>
          </div>{/if}
        <FormField
          label={`Enter ${resource.name} to confirm`}
        ...
        <Dialog.Footer
          ><Button
            type="button"
            variant="outline"
            disabled={dialogAction === "restore"}
            onclick={() => (restoreDialogOpen = false)}>Cancel</Button
          ><Button
            type="submit"
            variant="destructive"
            disabled={restoreConfirmation !== resource.name ||
              dialogAction === "restore"}
            aria-busy={dialogAction === "restore"}
            >{#if dialogAction === "restore"}<Spinner />{/if}Create safety
  ...
  <Dialog.Root bind:open={privateAccessDialogOpen}>
    <Dialog.Content>
      ...
      >
        <Dialog.Header
          ><Dialog.Title>Turn on WireGuard access</Dialog.Title
          ><Dialog.Description
            >Enable a private network for this Resource. You can then create
            endpoints routed through it.</Dialog.Description
          ></Dialog.Header
        >
        <FormField label="Private network" error={errors.privateNetworkId}
          ><NativeSelect.Root
            bind:value={privateAccessNetworkId}
            class="w-full"
            required
        ...
        <Dialog.Footer
          ><Button
            type="button"
            variant="outline"
            disabled={dialogAction === "private-access"}
            onclick={() => (privateAccessDialogOpen = false)}>Cancel</Button
          ><Button
            type="submit"
            disabled={dialogAction === "private-access"}
            aria-busy={dialogAction === "private-access"}
            >{#if dialogAction === "private-access"}<Spinner />{/if}Turn on
            access</Button
          ></Dialog.Footer
        >
      </form>
  ...
  <Dialog.Root bind:open={deviceDialogOpen}>
    <Dialog.Content>
      ...
      >
        <Dialog.Header
          ><Dialog.Title>Grant WireGuard access</Dialog.Title
          ><Dialog.Description
            >Grant an existing device or enroll a new one for this Resource.</Dialog.Description
          ></Dialog.Header
        >
        <FormField label="Existing device"
          ...
          ></FormField
        >
        {#if !device.deviceId}<FormField
            label="New device name"
            error={errors.name}
            ><Input
              bind:value={device.name}
              placeholder="MBV MacBook"
              required
            /></FormField
          >{/if}
        <Dialog.Footer
          ><Button
            ...
            onclick={() => (deviceDialogOpen = false)}>Cancel</Button
          ><Button
            type="submit"
            disabled={Boolean(pendingAction)}
            aria-busy={pendingAction === "device"}
            >{#if pendingAction === "device"}<Spinner />{/if}{device.deviceId
              ? "Grant access"
              : "Enroll and grant"}</Button
          ></Dialog.Footer
        >
      </form>
    </Dialog.Content>
      ...
      >
        <Dialog.Header
          ><Dialog.Title
            >{editingEndpointId
              ? "Edit endpoint"
              : "Add endpoint"}</Dialog.Title
          ><Dialog.Description
            >{editingEndpointId
              ? "Update how this Resource endpoint is published."
              : "Define another address for this Resource."}</Dialog.Description
          ></Dialog.Header
        >
        <div class="grid gap-4 sm:grid-cols-2">
          <FormField label="Name" error={errors.name}
            ><Input bind:value={endpoint.name} required /></FormField
          >
          <FormField label="Available to"
            ><NativeSelect.Root
              value={endpoint.audience}
              onchange={(event) =>
                chooseEndpointAudience(event.currentTarget.value)}
              class="w-full"
              ><NativeSelect.Option value="local_system"
                >Local system</NativeSelect.Option
              ><NativeSelect.Option value="environment"
                >Environments through WireGuard</NativeSelect.Option
              >{#if resource.engine !== "opentelemetry"}<NativeSelect.Option
                  value="custom">Custom address</NativeSelect.Option
                >{/if}</NativeSelect.Root
            ></FormField
          >
          {#if endpoint.audience === "environment"}
            <FormField label="Reach via" error={errors.privateNetworkId}
              ><NativeSelect.Root
                value={endpoint.privateNetworkId}
                onchange={(event) =>
                  chooseEndpointNetwork(event.currentTarget.value)}
                class="w-full"
                required
                ><NativeSelect.Option value=""
                  >Select a private network</NativeSelect.Option
                >{#each endpointNetworks as network}<NativeSelect.Option
                    ...
                    ] ?? "No Server address"}</NativeSelect.Option
                  >{/each}</NativeSelect.Root
              ></FormField
            >
          {:else if endpoint.audience === "local_system"}
            <div class="border border-border bg-muted/20 px-3 py-2">
              ...
              <p class="mt-1 font-mono text-sm">
                127.0.0.1 · this DeployCrate system
              </p>
            </div>
          {:else}
            ...
            >
          {/if}
          {#if serviceMappings.length > 0}<FormField
        ...
        {#if endpoint.publication.enabled || endpoint.protocol === "http" || endpoint.protocol === "https"}<div
            class="space-y-4 border border-border bg-muted/20 p-4"
          >
            <label class="flex items-center gap-3 text-sm"
              ><Checkbox bind:checked={endpoint.publication.enabled} /> Publish through
              Caddy</label
            >
            {#if endpoint.publication.enabled}<div
                class="grid gap-4 sm:grid-cols-2"
              >
                <FormField label="Public hostname" error={errors.hostname}
                  ><Input
                    bind:value={endpoint.publication.hostname}
                    required
                    placeholder="database.deploycrate.com"
                  /></FormField
                >{#if endpoint.protocol === "http" || endpoint.protocol === "https"}<FormField
                    label="Public health path"
                    error={errors.healthPath}
                    ><Input
                      bind:value={endpoint.publication.healthPath}
                ...
                <div class="sm:col-span-2 text-xs text-muted-foreground">
                  ...
                  >
                  will reverse proxy to
                  <span class="font-mono"
                    >{endpoint.address}:{endpoint.port}</span
                  >.
                </div>
              </div>{/if}
          </div>{/if}
        <Dialog.Footer
          >{#if editingEndpointId}<Button
              type="button"
              variant="destructive"
              disabled={dialogAction === "endpoint"}
              onclick={archiveEndpoint}>Archive</Button
            >{/if}<Button
            type="button"
            variant="outline"
            disabled={dialogAction === "endpoint"}
            onclick={() => (endpointDialogOpen = false)}>Cancel</Button
          ><Button
            type="submit"
            disabled={dialogAction === "endpoint"}
            aria-busy={dialogAction === "endpoint"}
            >{#if dialogAction === "endpoint"}<Spinner />{/if}{editingEndpointId
              ? "Save endpoint"
  ...
  <Dialog.Root bind:open={connectionKeysDialogOpen}>
    <Dialog.Content class="sm:max-w-2xl">
      ...
      >
        <Dialog.Header
          ><Dialog.Title
            >Injected keys for {connectionKeysConnection?.applicationName} / {connectionKeysConnection?.environmentName}</Dialog.Title
          ><Dialog.Description
            >These Resource-managed names apply only to this connection.
            Entering the Resource default restores inheritance for that key.</Dialog.Description
          ></Dialog.Header
        >
        <div class="grid gap-4 sm:grid-cols-2">
          {#each connectionKeyDefinitions(connectionKeysConnection) as key}
            ...
            >
              <Input
                value={connectionKeys[key.name] ??
                  resourceDefaultEnvironmentKey(key)}
                oninput={(event) =>
                  (connectionKeys[key.name] = event.currentTarget.value)}
                placeholder={resourceDefaultEnvironmentKey(key)}
                required
                autocomplete="off"
              />
              <p class="mt-1 text-[11px] text-muted-foreground">
                Default: <span class="font-mono"
                  >{resourceDefaultEnvironmentKey(key)}</span
                >
              </p>
            </FormField>
          {/each}
        ...
        <Dialog.Footer
          ><Button
            type="button"
            variant="outline"
            disabled={dialogAction === "connection-keys"}
            onclick={() => (connectionKeysDialogOpen = false)}>Cancel</Button
          ><Button
            type="submit"
            disabled={dialogAction === "connection-keys"}
            aria-busy={dialogAction === "connection-keys"}
            >{#if dialogAction === "connection-keys"}<Spinner />{/if}Save
            connection keys</Button
          ></Dialog.Footer
        >
      </form>
    </Dialog.Content>
      ...
      >
        <Dialog.Header
          ><Dialog.Title
            >{editingCredentialId
              ? "Edit credential"
              : "Add application credential"}</Dialog.Title
          ><Dialog.Description
            >{editingCredentialId
              ? "Update credential metadata or rotate its encrypted secret values."
              : "Create an encrypted application credential for this Resource."}</Dialog.Description
          ></Dialog.Header
        >
        <div class="grid gap-4 sm:grid-cols-2">
          <FormField label="Display name" error={errors.name}
            ><Input bind:value={credential.name} required /></FormField
          ><FormField label="Username" error={errors.username}
            ><Input
              bind:value={credential.username}
              readonly={Boolean(editingCredentialId) && managedPostgreSQL}
              required
              autocomplete="username"
            /></FormField
          >{#if databaseBacked}<FormField
              label="Database"
              error={errors["metadata.database"]}
              ><NativeSelect.Root
                bind:value={credential.database}
                class="w-full"
            ...
            >{/if}{#if !editingCredentialId || credential.rotate}{#each definition.credentialFields as field}<FormField
                ...
                ><Input
                  ...
                  value={credential.secretValues[field.name] ?? ""}
                  oninput={(event) =>
                    (credential.secretValues[field.name] =
        ...
        <Dialog.Footer
          >{#if editingCredentialId}<Button
              type="button"
              variant="destructive"
              disabled={dialogAction === "credential"}
              onclick={archiveCredential}>Archive</Button
            >{/if}<Button
            type="button"
            variant="outline"
            disabled={dialogAction === "credential"}
            onclick={() => (credentialDialogOpen = false)}>Cancel</Button
          ><Button
            type="submit"
            disabled={(!editingCredentialId && !canAddApplicationUser) ||
              dialogAction === "credential"}
            aria-busy={dialogAction === "credential"}
            >{#if dialogAction === "credential"}<Spinner
              />{/if}{editingCredentialId
              ? credential.rotate
                ? "Save and rotate"
                : "Save credential"
              : "Create application credential"}</Button
          ></Dialog.Footer
        >
      </form>
    </Dialog.Content>
  ...
  <Dialog.Root bind:open={backupPolicyDialogOpen}>
    <Dialog.Content class="sm:max-w-2xl">
      ...
      >
        <Dialog.Header
          ><Dialog.Title
            >{selectedBackups?.policy
              ? "Edit backup policy"
              : "Set up backups"}</Dialog.Title
          ><Dialog.Description
            >{selectedBackups?.databaseName
              ? `Configure backups for ${selectedBackups.databaseName}.`
              : "Configure backups for this Database."}</Dialog.Description
          ></Dialog.Header
        >
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <FormField label="Schedule" error={errors.schedule}
            ><Input
              bind:value={backupPolicy.schedule}
              class="font-mono"
              placeholder="0 2 * * *"
              required
            /></FormField
          >
          <FormField label="Keep last"
            ><Input
              type="number"
              min="0"
              bind:value={backupPolicy.keepLast}
            /></FormField
          >
          <FormField label="Keep daily"
            ><Input
              type="number"
              min="0"
              bind:value={backupPolicy.keepDaily}
            /></FormField
          >
          <FormField label="Keep weekly"
            ><Input
              type="number"
              min="0"
              bind:value={backupPolicy.keepWeekly}
            /></FormField
          >
          <FormField label="Keep monthly"
            ><Input
              type="number"
              min="0"
              bind:value={backupPolicy.keepMonthly}
            /></FormField
          >
        </div>
        <Dialog.Footer
          ><Button
            type="button"
            variant="outline"
            onclick={() => (backupPolicyDialogOpen = false)}>Cancel</Button
          ><Button type="submit"
            >{selectedBackups?.policy ? "Save policy" : "Create policy"}</Button
          ></Dialog.Footer
        >
      </form>
    </Dialog.Content>
      ...
      >
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          <div class="sm:col-span-2">
            <FormField label="Database name" error={errors.name}
              ><Input bind:value={database.name} required /></FormField
            >
          </div>
          <FormField label="Encoding" error={errors.encoding}
            ><Input bind:value={database.encoding} required /></FormField
          >
          <FormField label="Collation" error={errors.collation}
            ><Input
              bind:value={database.collation}
              placeholder="Default"
            /></FormField
          >
        </div>
  ...
  <Dialog.Root bind:open={volumeDialogOpen}>
    <Dialog.Content class="sm:max-w-xl">
      ...
      >
        <Dialog.Header
          ><Dialog.Title
            >{editingVolumeId ? "Edit volume" : "Add volume"}</Dialog.Title
          ><Dialog.Description
            >{editingVolumeId
              ? "Update this durable storage definition."
              : "Create durable storage for this Resource."}</Dialog.Description
          ></Dialog.Header
        >
        <div class="grid gap-4 sm:grid-cols-2">
          <FormField label="Name"
            ><Input bind:value={volume.name} required /></FormField
          ...
          >
          <div class="sm:col-span-2">
            <FormField label="Configuration JSON" error={jsonError}
              ><Textarea
                bind:value={volume.configurationText}
        ...
        <Dialog.Footer
          >{#if editingVolumeId}<Button
              type="button"
              variant="destructive"
              disabled={dialogAction === "volume"}
              onclick={archiveVolume}>Archive</Button
            >{/if}<Button
            type="button"
            variant="outline"
            disabled={dialogAction === "volume"}
            onclick={() => (volumeDialogOpen = false)}>Cancel</Button
          ><Button
            type="submit"
            disabled={dialogAction === "volume"}
            aria-busy={dialogAction === "volume"}
            >{#if dialogAction === "volume"}<Spinner />{/if}{editingVolumeId
              ? "Save volume"
  ...
  <Dialog.Root bind:open={mountDialogOpen}>
    <Dialog.Content class="sm:max-w-xl">
      ...
      >
        <Dialog.Header
          ><Dialog.Title
            >{editingMountId ? "Edit mount" : "Add mount"}</Dialog.Title
          ><Dialog.Description
            >Attach a Resource volume to an installation.</Dialog.Description
          ></Dialog.Header
        >
        <div class="grid gap-4 sm:grid-cols-2">
          <FormField label="Mount path"
            ><Input bind:value={mount.mountPath} required /></FormField
          ...
          ><label class="flex items-center gap-2 self-end text-xs"
            ><Checkbox bind:checked={mount.readOnly} /> Read only</label
          >
        </div>
        <Dialog.Footer
          >{#if editingMountId}<Button
              type="button"
              variant="destructive"
              disabled={dialogAction === "mount"}
              onclick={archiveMount}>Archive</Button
            >{/if}<Button
            type="button"
            variant="outline"
            disabled={dialogAction === "mount"}
            onclick={() => (mountDialogOpen = false)}>Cancel</Button
          ><Button
            type="submit"
            disabled={dialogAction === "mount"}
            aria-busy={dialogAction === "mount"}
            >{#if dialogAction === "mount"}<Spinner />{/if}{editingMountId
              ? "Save mount"
              : "Create mount"}</Button
          ></Dialog.Footer
        >
      ...
      >
        <Dialog.Header
          ><Dialog.Title
            >{editingHealthId
              ? "Edit health check"
              : "Add health check"}</Dialog.Title
          ><Dialog.Description
            >Define how DeployCrate should evaluate this Resource.</Dialog.Description
          ></Dialog.Header
        >
        <div class="grid gap-4 sm:grid-cols-2">
          <FormField label="Name"
          ...
          ><FormField label="Interval seconds"
            ><Input
              type="number"
              bind:value={health.intervalSeconds}
              min="1"
            /></FormField
          ><FormField label="Timeout seconds"
            ><Input
              type="number"
              bind:value={health.timeoutSeconds}
              min="1"
            /></FormField
          ><FormField label="Failure threshold"
            ><Input
              type="number"
              bind:value={health.failureThreshold}
              min="1"
            /></FormField
          ><FormField label="Success threshold"
            ><Input
              type="number"
              bind:value={health.successThreshold}
              min="1"
            /></FormField
          ><label class="flex items-center gap-2 text-xs"
            ><Checkbox bind:checked={health.enabled} /> Enabled</label
          >
          <div class="sm:col-span-2">
            <FormField label="Configuration JSON" error={jsonError}
              ><Textarea
                bind:value={health.configurationText}
                class="min-h-28 font-mono"
              /></FormField
            >
          </div>
        ...
        <Dialog.Footer
          >{#if editingHealthId}<Button
              type="button"
              variant="destructive"
              disabled={dialogAction === "health"}
              onclick={archiveHealth}>Archive</Button
            >{/if}<Button
            type="button"
            variant="outline"
            disabled={dialogAction === "health"}
            onclick={() => (healthDialogOpen = false)}>Cancel</Button
          ><Button
            type="submit"
            disabled={dialogAction === "health"}
            aria-busy={dialogAction === "health"}
            >{#if dialogAction === "health"}<Spinner />{/if}{editingHealthId
              ? "Save health check"
