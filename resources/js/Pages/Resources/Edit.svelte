<script lang="ts">
  ...
  function selectKind() { const selected = options.kinds.find((kind) => kind.kind === $identity.kind); if (selected) $identity.category = selected.category }
...
<DashboardLayout email={auth.email}>
  <div class="mx-auto max-w-5xl space-y-8">
    ...
    <Card.Root>
      <Card.Header><Card.Title>Docker installations</Card.Title><Card.Description>Configure the container artifact, placement, and loopback publication. The Resource kind owns the container port.</Card.Description></Card.Header>
