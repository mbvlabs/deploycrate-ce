<script lang="ts">
  ...
  const identity = useForm(() => ({ name: resource.name, slug: resource.slug, environmentKeys: initialEnvironmentKeys(resource.engine) }))
  const definition = $derived(options.engines.find((engine) => engine.engine === resource.engine) ?? options.engines[0])
  ...
  function saveIdentity(event: SubmitEvent) {
    ...
    $identity.transform((values) => ({ name: values.name, slug: values.slug, resourceType: resource.resourceType, configuration })).patch(routes.resourceUpdate(resource.id))
  }
...
<DashboardLayout email={auth.email} resourceNavigation={resource}>
  <div class="mx-auto max-w-6xl space-y-8">
    ...
    <form onsubmit={saveIdentity}>
      <div class="space-y-6">
        <Card.Root><Card.Header><Card.Title>Identity</Card.Title><Card.Description>How this Resource is named throughout DeployCrate.</Card.Description></Card.Header><Card.Content class="grid gap-5 sm:grid-cols-2"><FormField label="Name" error={errors.name}><Input value={$identity.name} oninput={(event) => updateName(event.currentTarget.value)} required /></FormField><FormField label="Slug" error={errors.slug}><Input value={$identity.slug} oninput={(event) => updateSlug(event.currentTarget.value)} required /></FormField></Card.Content></Card.Root>
