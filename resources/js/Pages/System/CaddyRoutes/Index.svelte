<script lang="ts">
  ...
  type ResourceRoute = { id: string; externalId: string; hostname: string; state: string; lastError: string; resourceId: string; resourceName: string; endpointName: string; origin: string; appliedAt: string; observedAt: string }
  ...
  <div class="space-y-8">
    <header class="flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        <p class="mt-2 max-w-2xl text-sm text-muted-foreground">DeployCrate manages these HTTPS routes and reconciles them to Caddy immediately.</p>
      </div>
          ...
          <Table.Body>
            {#each caddyRoutes as route (route.externalId)}
              <Table.Row>
                ...
                <Table.Cell>{route.applicationName} / {route.environmentName}</Table.Cell>
                <Table.Cell>Environment</Table.Cell>
                <Table.Cell><StatusBadge status={route.state} /></Table.Cell>
                <Table.Cell class="text-right"><Button size="sm" variant="outline">{#snippet child({ props })}<Link {...props} href={routes.caddyRouteShow(route.externalId)}>View</Link>{/snippet}</Button></Table.Cell>
              </Table.Row>
            {/each}
            {#each resourceRoutes as route (route.externalId)}
              <Table.Row>
                <Table.Cell class="font-medium">{route.hostname}</Table.Cell>
                <Table.Cell>{route.resourceName} / {route.endpointName}</Table.Cell>
  ...
  <Dialog.Root bind:open={showCreate}>
    <Dialog.Content class="sm:max-w-3xl">
      <form class="space-y-5" onsubmit={submitCreate}>
        <Dialog.Header><Dialog.Title>New Caddy route</Dialog.Title><Dialog.Description>Select one Environment domain and the running instances that should receive its traffic.</Dialog.Description></Dialog.Header>
        <div class="grid gap-5 sm:grid-cols-2">
          <FormField label="Domain"><NativeSelect.Root class="w-full" value={$form.environmentDomainId} onchange={(event) => selectCreateDomain(event.currentTarget.value)} required>{#each options.domains as domain}<NativeSelect.Option value={domain.id}>{domain.hostname} · {domain.applicationName} / {domain.environmentName}</NativeSelect.Option>{/each}</NativeSelect.Root></FormField>
          ...
          <FormField label="Target"><NativeSelect.Root class="w-full" value={$form.environmentTargetId} onchange={(event) => selectCreateTarget(event.currentTarget.value)} required>{#each createTargets as target}<NativeSelect.Option value={target.id}>{target.serverName}</NativeSelect.Option>{/each}</NativeSelect.Root></FormField>
          <FormField label="Active release"><NativeSelect.Root class="w-full" bind:value={$form.releaseId} required>{#each createReleases as release}<NativeSelect.Option value={release.id}>{release.label}</NativeSelect.Option>{/each}</NativeSelect.Root></FormField>
          <div class="sm:col-span-2"><p class="mb-2 text-xs font-medium">Backends and weights</p><div class="divide-y divide-border border border-border">{#each createInstances as instance}<div class="grid grid-cols-[auto_1fr_7rem] items-center gap-3 p-3"><Checkbox checked={backendSelected($form, instance.id)} onCheckedChange={(checked) => toggleBackend($form, instance.id, checked)} aria-label={`Use ${instance.externalId} as a backend`} /><div><p class="font-mono text-xs">{instance.externalId}</p><div class="mt-1 flex flex-wrap items-center gap-2"><span class="text-xs text-muted-foreground">{instance.address} · {instance.slot}</span><StatusBadge status={instance.state} /></div></div><Input type="number" min="0" max="100" disabled={!backendSelected($form, instance.id)} value={backendWeight($form, instance.id)} oninput={(event) => setBackendWeight($form, instance.id, Number(event.currentTarget.value))} /></div>{:else}<p class="p-4 text-sm text-muted-foreground">No active instances are available on this target.</p>{/each}</div><p class="mt-2 text-xs text-muted-foreground">Selected weights must total 100.</p></div>
        </div>
        <Dialog.Footer><Button type="button" variant="outline" disabled={$form.processing} onclick={() => (showCreate = false)}>Cancel</Button><Button type="submit" disabled={$form.processing || $form.backends.length === 0} aria-busy={$form.processing}>{#if $form.processing}<Spinner />{/if}Create and apply</Button></Dialog.Footer>
      </form>
    </Dialog.Content>
  </Dialog.Root>
</DashboardLayout>
