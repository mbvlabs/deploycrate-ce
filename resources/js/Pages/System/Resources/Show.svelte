					...
					{#if database.policy || database.history.length > 0}
						<Card.Content class="space-y-5">
							...
							{#if database.history.length > 0}
								<div class="overflow-hidden border border-border"><Table.Root><Table.Header><Table.Row><Table.Head>Status</Table.Head><Table.Head>Trigger</Table.Head><Table.Head>Scheduled</Table.Head><Table.Head>Size</Table.Head></Table.Row></Table.Header><Table.Body>{#each database.history as backup (backup.id)}<Table.Row><Table.Cell><div><StatusBadge status={backup.status} />{#if backup.error}<p class="mt-1 max-w-md text-xs text-destructive">{backup.error}</p>{/if}</div></Table.Cell><Table.Cell class="capitalize">{backup.triggerType}</Table.Cell><Table.Cell>{timestamp(backup.scheduledAt)}</Table.Cell><Table.Cell>{bytes(backup.sizeBytes)}</Table.Cell></Table.Row>{/each}</Table.Body></Table.Root></div>
							{/if}
						</Card.Content>
					{/if}
				</Card.Root>
