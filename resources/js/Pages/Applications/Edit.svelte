...
<DashboardLayout email={auth.email}><form class="mx-auto max-w-2xl" onsubmit={submit}><Card.Root><Card.Header><Card.Title>Edit application</Card.Title><Card.Description>Presentation details only. Source access is managed separately.</Card.Description></Card.Header><Card.Content class="grid gap-5"><div class="grid gap-2"><Label>Name</Label><Input bind:value={$form.name} required /></div><div class="grid gap-2"><Label>Slug</Label><Input bind:value={$form.slug} required /></div></Card.Content><Card.Footer class="border-t border-border"><Button type="submit" disabled={$form.processing}>Save changes</Button></Card.Footer></Card.Root></form></DashboardLayout>
