<script lang="ts">
  ...
  function healthy(environment: Environment) { return environment.sourceType === 'image' || (!environment.repositoryRemovedAt?.Valid && !environment.installationSuspendedAt?.Valid) }
</script>
  ...
  <div class="space-y-8">
    ...
    <div class="space-y-5">
      {#each application.environments as environment (environment.environmentId)}
        <Card.Root>
          <Card.Header>
            <Card.Action><span class:text-success={healthy(environment)} class:text-destructive={!healthy(environment)} class="text-xs">{healthy(environment) ? 'Source ready' : 'Source degraded'}</span></Card.Action>
