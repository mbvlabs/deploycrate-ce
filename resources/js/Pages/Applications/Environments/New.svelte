...
<DashboardLayout email={auth.email}>
  <form class="mx-auto max-w-5xl space-y-6" onsubmit={submit}>
    ...
    <Card.Root>
      ...
      <Card.Content class="space-y-5">
        <div class="grid gap-3 sm:grid-cols-2">
          <label class={`flex cursor-pointer gap-3 border p-4 transition-colors hover:border-primary/60 ${sourceType === 'buildpacks' ? 'border-primary bg-primary/5' : 'border-border'}`}><input class="mt-1" type="radio" name="source-type" value="buildpacks" bind:group={sourceType} /><span><span class="block text-sm font-medium">Buildpack</span><span class="mt-1 block text-xs leading-5 text-muted-foreground">Build source from GitHub and publish the resulting image.</span></span></label>
          <label class={`flex cursor-pointer gap-3 border p-4 transition-colors hover:border-primary/60 ${sourceType === 'image' ? 'border-primary bg-primary/5' : 'border-border'}`}><input class="mt-1" type="radio" name="source-type" value="image" bind:group={sourceType} /><span><span class="block text-sm font-medium">Repository</span><span class="mt-1 block text-xs leading-5 text-muted-foreground">Deploy directly from an existing container repository.</span></span></label>
        </div>
        {#if sourceType === 'buildpacks'}
          <div class="grid gap-5 border-t border-border pt-5 sm:grid-cols-2">
            <FormField label="GitHub account"><select value={githubInstallationId} onchange={(event) => chooseInstallation(event.currentTarget.value)} class="h-9 w-full border border-input bg-background px-3 text-sm" required><option value="">Select an account</option>{#each installations as installation}<option value={installation.id}>{installation.accountLogin}</option>{/each}</select></FormField>
            <FormField label="Source repository"><select value={githubRepositoryId} onchange={(event) => chooseRepository(event.currentTarget.value)} class="h-9 w-full border border-input bg-background px-3 text-sm" required><option value="">Select a repository</option>{#each repositoriesForInstallation as repository}<option value={repository.id}>{repository.fullName}</option>{/each}</select></FormField>
            <FormField label="Git reference"><Input bind:value={reference} placeholder="main" required /></FormField>
            <FormField label="Build server"><select bind:value={buildServerId} class="h-9 w-full border border-input bg-background px-3 text-sm" required><option value="">Select a Build Server</option>{#each buildServers as server}<option value={server.id}>{server.name} · {server.kind === 'worker' ? server.address : 'Control plane'}</option>{/each}</select></FormField>
            ...
            <FormField label="Registry"><select bind:value={registryResourceId} class="h-9 w-full border border-input bg-background px-3 text-sm" required><option value="">Select a Registry</option>{#each registries as registry}<option value={registry.id}>{registry.name} · {registry.endpoint}</option>{/each}</select></FormField>
            <label class="flex items-center gap-2 self-end pb-2 text-sm"><input type="checkbox" bind:checked={autoBuild} /> Build automatically on matching pushes</label>
            <label class="flex items-start gap-3 border border-border p-4 sm:col-span-2"><input class="mt-1" type="checkbox" bind:checked={buildFrontendAssets} /><span><span class="block text-sm font-medium">Build Node frontend assets</span><span class="mt-1 block text-xs text-muted-foreground">Requires a supported lockfile and build script.</span></span></label>
          </div>
        {:else}
          <div class="grid gap-5 border-t border-border pt-5 sm:grid-cols-2"><FormField label="Registry"><select bind:value={registryResourceId} class="h-9 w-full border border-input bg-background px-3 text-sm" required><option value="">Select a Registry</option>{#each registries as registry}<option value={registry.id}>{registry.name} · {registry.endpoint}</option>{/each}</select></FormField><FormField label="Repository"><Input bind:value={imageRepository} placeholder="team/application" required /></FormField><FormField label="Tag or digest"><Input bind:value={reference} placeholder="stable" required /></FormField></div>
        {/if}
      </Card.Content>
    </Card.Root>
    ...
    <Card.Root>
      ...
      <Card.Content class="space-y-5">
        {#if servers.length === 0}<div class="flex items-center justify-between gap-4 border border-border p-4"><p class="text-sm text-muted-foreground">No runtime target servers are available.</p><Button type="button" href={routes.nodeNew()} variant="outline">Add node</Button></div>{:else}<div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{#each servers as server}<label class={`flex cursor-pointer gap-3 border p-4 transition-colors hover:border-primary/60 ${serverIds.includes(server.id) ? 'border-primary bg-primary/5' : 'border-border'}`}><input class="mt-1" type="checkbox" checked={serverIds.includes(server.id)} onchange={(event) => toggleServer(server.id, event.currentTarget.checked)} /><span><span class="block text-sm font-medium">{server.name}</span><span class="mt-1 block text-xs text-muted-foreground">{server.kind === 'worker' ? server.address : 'Control plane'}</span></span></label>{/each}</div>{/if}
    ...
    <Card.Root>
      ...
      <Card.Content class="space-y-4"><div class="flex gap-2"><select bind:value={selectedResource} class="h-9 flex-1 border border-input bg-background px-3 text-sm"><option value="">Select a Resource</option>{#each availableResources as option}<option value={`${option.id}:${option.endpointId}:${option.credentialId ?? ''}`}>{option.name} · {option.engine}{option.database ? ` · ${option.database}` : ''} · {option.endpoint} · {option.credential || 'without credentials'}</option>{/each}</select><Button type="button" variant="outline" onclick={addResource}>Attach</Button></div>{#each resources as resource, index}<div class="grid gap-3 border border-border p-4 sm:grid-cols-2"><FormField label="Alias"><Input bind:value={resource.alias} /></FormField>{#if resource.database}<FormField label="Database"><Input bind:value={resource.database} readonly /></FormField>{/if}{#if attachedResourceOption(resource)?.supportsConnectionUrl}<FormField label="Connection format"><select bind:value={resource.credentialProjection} class="h-9 w-full border border-input bg-background px-3 text-sm"><option value="connection_url">Connection URL</option><option value="individual_parts">Individual parts</option></select></FormField>{/if}<Button type="button" variant="ghost" onclick={() => resources = resources.filter((_, itemIndex) => itemIndex !== index)}>Remove</Button></div>{:else}<p class="text-xs text-muted-foreground">No Resources attached.</p>{/each}</Card.Content>
    </Card.Root>
    ...
    <label class="flex items-center gap-3 border border-border bg-muted/20 p-4"><input type="checkbox" bind:checked={deploy} /><span class="text-sm font-medium">Deploy after create</span></label>

    <div class="flex justify-end gap-3 border-t border-border pt-5"><Button href={routes.applicationShow(application.id)} variant="outline">Cancel</Button><Button type="submit" disabled={processing || registries.length === 0 || serverIds.length === 0}>{processing ? 'Adding Environment...' : 'Add Environment'}</Button></div>
  </form>
