<script lang="ts">
  ...
  type Secret = { id: string; key: string; digestPrefix: string; sourceType: string; sourceId: string; createdAt: string; status: 'deployed' | 'deploying' | 'pending' | 'failed' | 'pending_removal'; desired: boolean }
  ...
  const secretSourceLabel = (secret: Secret) => {
    ...
    const resource = environment.resources.find((candidate) => candidate.id === secret.sourceId)
    return resource ? `Resource managed by ${resource.name}` : 'Resource managed'
  }
