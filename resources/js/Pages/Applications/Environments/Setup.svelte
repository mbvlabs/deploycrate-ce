<script lang="ts">
  ...
  let { auth, environment, options, setupUrl, setupError = '' }: { auth: { email: string }; environment: Overview; options: { resources: ResourceOption[] }; setupUrl: string; setupError?: string } = $props()
  let step = $state(1)
  ...
  const form = useForm(() => ({ hostname: '', containerPort: 8080, healthPath: '/health', bpGoTargets: '', resources: [] as Array<{ resourceId: string; endpointId: string; credentialId?: string; alias: string; database: string; credentialProjection: 'connection_url' | 'individual_parts' }>, secrets: [] as Array<{ key: string; value: string }> }))
  ...
  function showErrorStep(errors: Record<string, unknown>) {
    ...
    else if (fields.some((field) => ['containerPort', 'healthPath', 'bpGoTargets'].includes(field))) step = 2
    ...
    <form onsubmit={submit}>
      <Card.Root>
        ...
        <Card.Content class="space-y-5">
          ...
          {:else if step === 2}
            <div class="grid gap-5 sm:grid-cols-2"><FormField label="Container port" error={$form.errors.containerPort}><Input type="number" min="1" max="65535" bind:value={$form.containerPort} required /></FormField><FormField label="HTTP health path" error={$form.errors.healthPath}><Input bind:value={$form.healthPath} placeholder="/health" /></FormField><FormField label="BP_GO_TARGETS" error={$form.errors.bpGoTargets}><Input bind:value={$form.bpGoTargets} placeholder="./cmd/server" /></FormField></div>
          {:else if step === 3}
            <div class="flex gap-2"><select bind:value={selectedResource} class="h-9 flex-1 border border-input bg-background px-3 text-sm"><option value="">Select an existing PostgreSQL Resource</option>{#each options.resources as option}<option value={`${option.id}:${option.endpointId}:${option.credentialId ?? ''}`}>{option.name} · {option.database} · {option.endpoint} · {option.credential || 'No credential'}</option>{/each}</select><Button type="button" variant="outline" onclick={addResource}>Attach</Button></div>
          ...
          {:else}
            <div class="grid gap-4 text-sm sm:grid-cols-2"><p><span class="text-muted-foreground">Domain</span><br />{$form.hostname}</p><p><span class="text-muted-foreground">Runtime</span><br />Go · port {$form.containerPort}</p><p><span class="text-muted-foreground">Resources</span><br />{$form.resources.length}</p><p><span class="text-muted-foreground">Secret keys</span><br />{$form.secrets.map((secret) => secret.key).join(', ') || 'None'}</p></div><p class="text-xs text-muted-foreground">Submitting resolves the configured GitHub ref to an exact commit and atomically queues the first build. Secret values are write-only.</p>
          {/if}
        </Card.Content>
