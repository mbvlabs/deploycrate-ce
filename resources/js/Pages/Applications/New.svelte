<script lang="ts">
  ...
  function attachedResourceOption(resource: EnvironmentResource) {
    return resources.find((option) => option.id === resource.resourceId && option.endpointId === resource.endpointId && option.credentialId === resource.credentialId)
      ?? resources.find((option) => option.id === resource.resourceId && option.endpointId === resource.endpointId && option.engine === 'opentelemetry')
  }
  ...
  <Card.Root>
    ...
    <Card.Content class="space-y-6">
      ...
      <section class="space-y-3 border-t border-border pt-5">
        ...
        <div class="flex flex-col gap-2 sm:flex-row"><NativeSelect.Root value={environment.selectedResource} onchange={(event) => environment.selectedResource = event.currentTarget.value} class="w-full flex-1"><NativeSelect.Option value="">Select a Resource</NativeSelect.Option>{#each availableResources(environment) as option}<NativeSelect.Option value={`${option.id}:${option.endpointId}:${option.credentialId ?? ''}`}>{option.name} · {option.engine}{option.database ? ` · ${option.database}` : ''} · {option.endpoint} · {option.credential || (option.engine === 'opentelemetry' ? 'Environment credential created on attach' : 'without credentials')}</NativeSelect.Option>{/each}</NativeSelect.Root><Button type="button" variant="outline" disabled={!environment.selectedResource} onclick={() => addResource(environment)}>Attach</Button></div>
