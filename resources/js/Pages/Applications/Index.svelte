              ...
              ><Card.Action
                ><StatusBadge
                  ...
                  label={applicationStatus(application) === "ready"
                    ...
                    : "Source degraded"}
                /></Card.Action
              ><Card.Title>{application.name}</Card.Title><Card.Description
                >{application.environments.length}
                {application.environments.length === 1
                  ? "environment"
                  : "environments"} · {application.slug}</Card.Description
              ></Card.Header
            >
            <Card.Content>
              {#if application.environments.length > 0}
                <div class="divide-y divide-border border border-border">
                  {#each application.environments as environment (environment.id)}<div
                    ...
                    >
                      <div class="min-w-0">
                        ...
                        <p
                          class="mt-1 capitalize text-xs text-muted-foreground"
                        >
                          {environment.environmentKind} · {environment.sourceType}
                        </p>
                        <p
                          class="mt-1 truncate font-mono text-xs text-muted-foreground"
                        >
                          {environment.repositoryFullName || "Source pending"} · {environment.reference ||
                            "Reference pending"}
                        </p>
                      </div>
                      <StatusBadge
                        status={environment.sourceHealthy
                          ? "ready"
                          : "degraded"}
                        label={environment.sourceHealthy
                          ? "Source ready"
                          : "Source degraded"}
                      />
                    </div>{/each}
              ...
              {:else}
                <Empty.Root class="border border-dashed border-border py-8"
                  ><Empty.Header
                    ><Empty.Title>No environments</Empty.Title
                    ><Empty.Description
                      >Add an environment to make this application deployable.</Empty.Description
                    ></Empty.Header
                  ></Empty.Root
                >
              {/if}
