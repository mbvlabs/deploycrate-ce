<script lang="ts">
  ...
  function rotate(event: SubmitEvent) {
    ...
    rotateError = "";
    router.patch(
      routes.dnsConnectionTokenUpdate(rotateTarget.id),
      { token: rotateToken },
      {
        preserveScroll: true,
        onSuccess: () => {
...
<DashboardLayout email={auth.email}>
  <div class="space-y-8">
    ...
    {:else}
      <Card.Root>
        <Card.Header
          ><Card.Title>Cloudflare accounts</Card.Title><Card.Description
            >{connections.length} connected account{connections.length === 1
              ? ""
              : "s"}. Zones are cached locally and refreshed explicitly.</Card.Description
          ></Card.Header
        >
        <Card.Content>
          <div class="overflow-hidden border border-border">
            <Table.Root class="min-w-[980px]">
              <Table.Header
                ><Table.Row
                  ><Table.Head>Connection</Table.Head><Table.Head
                    >Cloudflare account</Table.Head
                  ><Table.Head>Usage</Table.Head><Table.Head
                    >Verified</Table.Head
                ...
                {#each connections as connection (connection.id)}
                  <Table.Row>
                    ...
                    <Table.Cell class="font-mono text-xs"
                      >{connection.accountId}</Table.Cell
                    >
                    <Table.Cell
                      ><p>
                        {connection.activeZones} active zone{connection.activeZones ===
                        1
                          ? ""
                          : "s"}
                      </p>
                      <p class="mt-1 text-[11px] text-muted-foreground">
                        {connection.bindingCount} managed domain{connection.bindingCount ===
                        1
                          ? ""
                          : "s"}
                      </p></Table.Cell
                    ...
                    <Table.Cell class="whitespace-nowrap"
                      >{dateLabel(connection.verifiedAt)}</Table.Cell
                    >
                    <Table.Cell class="whitespace-nowrap"
                    ...
                    <Table.Cell
                      ><div class="flex justify-end gap-2">
                        <Button
                          ...
                          >{#if activeAction === `sync:${connection.id}`}<Spinner
                            />{:else}<RefreshCwIcon />{/if}Sync</Button
                        ><Button
                          size="sm"
                          variant="outline"
                          disabled={Boolean(activeAction)}
                          onclick={() => openRotate(connection)}
                          >Rotate token</Button
                        ><Button
                          size="sm"
                          variant="destructive"
                          disabled={Boolean(activeAction)}
                          onclick={() => askArchive(connection)}>Archive</Button
                        >
                      </div></Table.Cell
                    >
                  </Table.Row>
      ...
      <form class="grid gap-5" onsubmit={rotate}>
        <Dialog.Header
          ><Dialog.Title>Rotate API token</Dialog.Title><Dialog.Description
            >Replace the account-owned API token for {rotateTarget?.name ??
              "this connection"}. The new token is verified before it replaces
            the current credential.</Dialog.Description
          ></Dialog.Header
        >
        <FormField label="New account-owned API token"
          ><Input
            ...
            autocomplete="new-password"
            required
            disabled={activeAction.startsWith("rotate:")}
          /></FormField
        >
        {#if rotateError}<p
        ...
        <Dialog.Footer
          ...
          ><Button
            type="submit"
            disabled={!rotateToken.trim() || activeAction.startsWith("rotate:")}
            >{#if activeAction.startsWith("rotate:")}<Spinner />{/if}Rotate
            token</Button
          ></Dialog.Footer
        >
      </form>
