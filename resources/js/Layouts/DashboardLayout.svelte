<script lang="ts">
  ...
  const breadcrumbs = $derived.by(() => {
    ...
    if (
      resourceNavigation &&
      contextualResourceRoutes &&
      Object.values(contextualResourceRoutes).includes(path)
    ) {
      ...
      return [
        { label: "Resources", href: routes.resources() },
        {
          ...
          href: contextualResourceRoutes.overview,
        },
        ...
        <Sidebar.Group>
          ...
          <Sidebar.GroupContent>
            <Sidebar.Menu>
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url === routes.homePage()}
                  tooltipContent="Home"
                >
                  {#snippet child({ props })}
                    <Link
                      {...props}
                      href={routes.homePage()}
                      aria-current={$page.url === routes.homePage()
                        ? "page"
                        : undefined}
                    >
                      <LayoutDashboardIcon />
              ...
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.applications())}
                  tooltipContent="Applications"
                >
                  {#snippet child({ props })}<Link
                      {...props}
                      href={routes.applications()}
                      aria-current={$page.url.startsWith(routes.applications())
                        ? "page"
                        : undefined}
                      ><AppWindowIcon /><span>Applications</span></Link
                    >{/snippet}
                </Sidebar.MenuButton>
              </Sidebar.MenuItem>
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.resources())}
                ...
                >
                  {#snippet child({ props })}
                    <Link
                      {...props}
                      href={routes.resources()}
                      aria-current={$page.url.startsWith(routes.resources())
                        ? "page"
                        : undefined}
                    >
                      <DatabaseIcon />
        ...
        <Sidebar.Group>
          ...
          <Sidebar.GroupContent>
            <Sidebar.Menu>
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.nodes())}
                  tooltipContent="Nodes"
                >
                  {#snippet child({ props })}<Link
                      {...props}
                      href={routes.nodes()}
                      aria-current={$page.url.startsWith(routes.nodes())
                        ? "page"
                        : undefined}><ServerIcon /><span>Nodes</span></Link
                    >{/snippet}
                </Sidebar.MenuButton>
              </Sidebar.MenuItem>
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.networks())}
                  tooltipContent="Networks"
                >
                  {#snippet child({ props })}
                    <Link
                      {...props}
                      href={routes.networks()}
                      aria-current={$page.url.startsWith(routes.networks())
                        ? "page"
                        : undefined}
                    >
                      <NetworkIcon />
              ...
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.caddyRoutes())}
                  tooltipContent="Caddy Routes"
                >
                  {#snippet child({ props })}<Link
                      {...props}
                      href={routes.caddyRoutes()}
                      aria-current={$page.url.startsWith(routes.caddyRoutes())
                        ? "page"
                        : undefined}
                      ><RouteIcon /><span>Caddy Routes</span></Link
                    >{/snippet}
                </Sidebar.MenuButton>
              </Sidebar.MenuItem>
            </Sidebar.Menu>
        ...
        <Sidebar.Group>
          ...
          <Sidebar.GroupContent>
            <Sidebar.Menu>
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.registryResources())}
                  tooltipContent="Image Registry"
                >
                  {#snippet child({ props })}<Link
                      {...props}
                      href={routes.registryResources()}
                      aria-current={$page.url.startsWith(
                        routes.registryResources(),
                      )
                        ? "page"
                        : undefined}
                      ><KeyRoundIcon /><span>Image Registry</span></Link
                    >{/snippet}
                </Sidebar.MenuButton>
              </Sidebar.MenuItem>
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.objectStorage())}
                  tooltipContent="Object Storage"
                >
                  {#snippet child({ props })}<Link
                      {...props}
                      href={routes.objectStorage()}
                      aria-current={$page.url.startsWith(routes.objectStorage())
                        ? "page"
                        : undefined}
                      ><CloudIcon /><span>Object Storage</span></Link
                    >{/snippet}
                </Sidebar.MenuButton>
              </Sidebar.MenuItem>
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.dnsConnections())}
                  tooltipContent="DNS"
                >
                  {#snippet child({ props })}<Link
                      {...props}
                      href={routes.dnsConnections()}
                      aria-current={$page.url.startsWith(
                        routes.dnsConnections(),
                      )
                        ? "page"
                        : undefined}><NetworkIcon /><span>DNS</span></Link
                    >{/snippet}
                </Sidebar.MenuButton>
              </Sidebar.MenuItem>
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.gitHubConnection())}
                  tooltipContent="GitHub"
                >
                  {#snippet child({ props })}<Link
                      {...props}
                      href={routes.gitHubConnection()}
                      aria-current={$page.url.startsWith(
                        routes.gitHubConnection(),
                      )
                        ? "page"
                        : undefined}><GithubIcon /><span>GitHub</span></Link
                    >{/snippet}
                </Sidebar.MenuButton>
              </Sidebar.MenuItem>
            </Sidebar.Menu>
        ...
        <Sidebar.Group>
          ...
          <Sidebar.GroupContent>
            <Sidebar.Menu>
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url === routes.systemOverview()}
                  tooltipContent="Overview"
                >
                  {#snippet child({ props })}
                    <Link
                      {...props}
                      href={routes.systemOverview()}
                      aria-current={$page.url === routes.systemOverview()
                        ? "page"
                        : undefined}
                    >
                      <ShieldCheckIcon />
              ...
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.systemTelemetry())}
                  tooltipContent="Telemetry"
                >
                  {#snippet child({ props })}
                    <Link
                      {...props}
                      href={routes.systemTelemetry()}
                      aria-current={$page.url.startsWith(
              ...
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.systemTasks())}
                  tooltipContent="System Tasks"
                >
                  {#snippet child({ props })}
                    <Link
                      {...props}
                      href={routes.systemTasks()}
                      aria-current={$page.url.startsWith(routes.systemTasks())
                        ? "page"
                        : undefined}
                    >
                      <ListTodoIcon />
              ...
              <Sidebar.MenuItem>
                <Sidebar.MenuButton
                  isActive={$page.url.startsWith(routes.systemUpdate())}
                  tooltipContent="Updates"
                >
                  {#snippet child({ props })}
                    <Link
                      {...props}
                      href={routes.systemUpdate()}
                      aria-current={$page.url.startsWith(routes.systemUpdate())
                        ? "page"
                        : undefined}
                    >
                      <SettingsIcon />
          ...
          <Breadcrumb.Root class="hidden min-w-0 sm:block">
            <Breadcrumb.List class="flex-nowrap">
              {#each breadcrumbs as crumb, index (crumb.label)}
                ...
                {#if index < breadcrumbs.length - 1}<Breadcrumb.Separator
                  />{/if}
              {/each}
            </Breadcrumb.List>
          </Breadcrumb.Root>
