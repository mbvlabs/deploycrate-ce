<script lang="ts">
  ...
  let {
    ...
    class: className,
    expanded = false,
  }: { value: unknown; class?: string; expanded?: boolean } = $props();
  ...
  function tokenize(input: string): Token[] {
    ...
    return matches.map((match) => {
      ...
      if (/^(true|false|null)$/.test(token))
        return { value: token, kind: "literal" };
      if (/^-?\d/.test(token)) return { value: token, kind: "number" };
      if (/^[{}\[\],:]$/.test(token))
        return { value: token, kind: "punctuation" };
...
<div
  class={cn(
    "relative min-w-0 max-w-full border border-border bg-muted/20",
    className,
  )}
>
  <Button
    ...
    variant="ghost"
    size="icon-xs"
    class="absolute right-2 top-2 z-10 bg-background/80"
    aria-label="Copy JSON"
    title="Copy JSON"
    onclick={copy}><CopyIcon /></Button
  >
  <ScrollArea.Root
    orientation="both"
    class={cn(expanded ? "max-h-none" : "max-h-96", "w-full")}
  >
    <pre class="min-w-max whitespace-pre p-4 pr-10 text-xs leading-5"><code
        >{#each tokens as token}<span class={tokenClass(token.kind)}
            >{token.value}</span
          >{/each}</code
      ></pre>
  </ScrollArea.Root>
