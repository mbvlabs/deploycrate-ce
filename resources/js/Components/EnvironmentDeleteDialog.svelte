...
<AlertDialog.Root {open} onOpenChange={setOpen}>
  ...
  <AlertDialog.Content>
    <form class="grid gap-4" onsubmit={destroy}>
      <AlertDialog.Header>
        <AlertDialog.Title
          >Permanently delete {environmentName}?</AlertDialog.Title
        >
        <AlertDialog.Description
          >This removes the Environment, its builds, deployments, releases,
          secrets, routes, containers, network, and background jobs. This cannot
          be undone.</AlertDialog.Description
        >
      </AlertDialog.Header>
      <Field.Field data-invalid={Boolean(requestError)}>
        <Field.Label class="w-full flex-col items-start">
          <span
            >Type <span class="font-mono text-foreground"
        ...
        {#if requestError}<Field.Error
            class="whitespace-pre-wrap border border-destructive/50 bg-destructive/10 p-3"
            >{requestError}</Field.Error
          >{/if}
      </Field.Field>
      <AlertDialog.Footer>
        <Button
