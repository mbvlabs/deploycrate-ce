...
<script lang="ts">
  ...
  const updateProgress = $derived.by(() => {
    ...
    const index = steps.findIndex((step) =>
      update.currentStep.toLowerCase().includes(step),
    );
  ...
  <Dialog.Content class="sm:max-w-3xl" showCloseButton={!running && !starting}>
    <Dialog.Header>
      <div class="flex flex-wrap items-center gap-3 pr-8">
        <Dialog.Title
          >{tracking
            ? "DeployCrate CE update"
            : "Update DeployCrate CE"}</Dialog.Title
        >
        {#if tracking}
          <StatusBadge
      ...
      <Dialog.Description>
        {#if tracking}
          Follow the blue-green deployment and traffic cutover without leaving
          this dialog.
        {:else}
    ...
    {#if !tracking}
      <div class="space-y-5">
        ...
        <p class="text-sm leading-6 text-muted-foreground">
          ...
          inactive slot, switches traffic only after health validation, and
          restores the current binary if validation fails.
        </p>
      </div>
      <Dialog.Footer>
      ...
      <div class="space-y-5">
        {#if !starting && update.state === "failed"}
          <Alert.Root variant="destructive">
            <AlertTriangleIcon />
            <Alert.Title
              >Update failed during {stepLabel(update.currentStep)}</Alert.Title
            >
        ...
        {:else if !starting && update.state === "succeeded"}
          <Alert.Root>
            ...
            <Alert.Description>
              DeployCrate CE is serving from the {update.activeInstance} instance
              on {versionLabel(update.targetVersion)}.
            </Alert.Description>
          </Alert.Root>
        {/if}
          ...
          <div class="space-y-2">
            <div class="flex justify-between gap-3 text-xs">
              <span class="flex items-center gap-2 capitalize"
                ><Spinner />{starting && !running
                  ? "Starting update"
                  : stepLabel(update.currentStep)}</span
              >
              <span>{starting && !running ? 0 : updateProgress}%</span>
            </div>
            <Progress
              value={starting && !running ? 0 : updateProgress}
            ...
            <Empty.Root class="py-8">
              <Empty.Header>
                <Empty.Title
                  >{starting
                    ? "Starting update"
                    : "No update activity"}</Empty.Title
                >
                <Empty.Description
                  >Deployment and cutover events will appear here as they are
                  recorded.</Empty.Description
                >
              </Empty.Header>
            </Empty.Root>
          {:else}
                ...
                {#each [...visibleEvents].reverse() as event, index (event.id)}
                  <li class="grid grid-cols-[0.75rem_1fr] gap-3">
                    <div class="flex flex-col items-center">
                      <span
                        class="mt-1.5 size-2 border border-primary bg-primary/20"
                      ></span>
                      {#if index < visibleEvents.length - 1}<span
                    ...
                    <div class="pb-4">
                      ...
                      <p
                        class="mt-1 text-[10px] uppercase tracking-[0.12em] text-muted-foreground"
                      >
