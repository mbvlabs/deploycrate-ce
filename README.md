...
The shell installer downloads the architecture-specific bootstrap binary from the stable R2 channel, verifies its SHA-256 checksum, installs it under `/usr/local/bin`, and opens the wizard through `/dev/tty`. The wizard then resolves and installs the application from the selected channel manifest. Without a usable TTY, the shell installer prints the command needed to continue. `DEPLOYCRATE_INSTALLER_BASE_URL` can select a compatible bootstrap mirror.
...
GitHub Actions publishes successful `master` builds to the edge R2 bucket and version tags to the stable R2 bucket. Both AMD64 and ARM64 are supported. `just development-assets` creates the same edge artifact layout locally without publishing it.

> [!WARNING]
> Release manifests currently authenticate artifacts with SHA-256 but are not yet signed. Signed manifest verification is required before declaring the stable channel production-ready.
...
Each R2 channel publishes `manifest.json` last, after immutable architecture-specific artifacts are available. Bootstrap persists the selected `stable` or `edge` channel, and the updater downloads the exact Linux AMD64 or ARM64 artifact referenced by that channel manifest. It verifies the manifest checksum and requires the staged binary's `version` output to match the manifest before installation.
