...
DeployCrate accepts Go applications with a `go.mod` in the configured context. The Build worker downloads the exact private GitHub archive, invokes the pinned Paketo Ubuntu Noble builder, Go buildpack, and run image, publishes to the authenticated managed Distribution registry, verifies the immutable digest, and queues a direct Docker deployment. Each Environment owns one schema-versioned Pack build cache volume and one launch cache volume. Warm Builds reuse those volumes and analyze the latest successful immutable Release image while retaining a unique output tag for every Build. Pack uses `if-not-present` for the digest-pinned inputs, and Build logs record queue wait plus source download, extraction, registry authentication, Pack execution, digest resolution, and finalization durations. Permanently deleting an Environment removes only its two exact cache volumes. Environment workloads use an ownership-labeled bridge and container, `unless-stopped`, and a dynamic port bound only to `127.0.0.1`. They do not use Docker Compose or per-Environment systemd units. Installation-backed Docker Resources are attached to each consuming Environment network. Host-native processes use the Resource's loopback-published port, while Environment workloads use the Resource container name and internal container port.
