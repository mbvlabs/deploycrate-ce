...
func (service *ServerExecution) Target(ctx context.Context, serverID uuid.UUID, capability models.ServerCapability) (ServerExecutionTarget, error) {
...
func (service *ServerExecution) RunRootScript(ctx context.Context, target ServerExecutionTarget, script []byte) (sshclient.Result, error) {
	return service.run(ctx, target, "sudo -n /bin/bash -s", bytes.NewReader(script))
}

func (service *ServerExecution) RunRootCommand(ctx context.Context, target ServerExecutionTarget, input io.Reader, executable string, arguments ...string) (sshclient.Result, error) {
	return service.run(ctx, target, "sudo -n -- "+shellJoin(executable, arguments...), input)
}

func (service *ServerExecution) RunRootCommandStreaming(ctx context.Context, target ServerExecutionTarget, input io.Reader, output io.Writer, executable string, arguments ...string) error {
	if !target.Remote {
	...
	err = service.ssh.RunWithCertificateStreaming(ctx, address, "admin", target.Credential.KnownHostKey, certificate.PrivateKey, certificate.Certificate,
		"sudo -n -- "+shellJoin(executable, arguments...), input, output, &stderr)
	if err != nil {
...
func (service *ServerExecution) run(ctx context.Context, target ServerExecutionTarget, command string, input io.Reader) (sshclient.Result, error) {
	if !target.Remote {
	...
	address := net.JoinHostPort(target.Peer.PrivateAddress, strconv.Itoa(int(target.Credential.Port)))
	return service.ssh.RunWithCertificate(ctx, address, "admin", target.Credential.KnownHostKey, certificate.PrivateKey, certificate.Certificate, command, input)
}
