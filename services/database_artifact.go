...
func (service *DatabaseArtifact) Load(
	...
	if scope.Backup.ResourceID == nil {
		return fail(errors.New("database backup record is missing its Resource identity"))
	}
	expectedMetadata := map[string]string{
		"backup-id": scope.Backup.ID.String(), "policy-id": scope.Backup.BackupPolicyID.String(),
		"resource-id": scope.Backup.ResourceID.String(), "resource-installation-id": target.InstallationID.String(),
