...
func (service CaddyRouteService) CustomRouteSnapshot(
	ctx context.Context,
) ([]ManagedCustomCaddyRoute, error) {
...
func managedCustomCaddyRoute(row models.CustomCaddyRouteEntity) ManagedCustomCaddyRoute {
	...
	return ManagedCustomCaddyRoute{
		...
		Origin: fmt.Sprintf(
			"%s://%s",
			protocol,
			net.JoinHostPort(row.OriginAddress, fmt.Sprint(row.OriginPort)),
		),
		OriginAddress:  row.OriginAddress,
		OriginPort:     row.OriginPort,
		OriginProtocol: row.OriginProtocol,
		OriginTLSMode:  row.OriginTLSMode,
		HealthPath:     row.HealthPath,
		State:          row.State.String(),
		LastError:      row.LastError.String,
		AppliedAt: nullableTimeString(
			row.AppliedAt,
		),
...
func (service CaddyRouteService) CreateCustom(
	ctx context.Context,
	input CustomCaddyRouteInput,
) (uuid.UUID, error) {
	entity, err := service.validateCustomRoute(ctx, uuid.Nil, input)
	if err != nil {
		return uuid.Nil, err
	}
	created, err := models.CustomCaddyRoute.Create(
		ctx,
		service.db.Executor(),
		models.SaveCustomCaddyRouteData{
...
func (service CaddyRouteService) UpdateCustom(
	ctx context.Context,
	id uuid.UUID,
	input CustomCaddyRouteInput,
) error {
	...
	updated, err := models.CustomCaddyRoute.Update(
		ctx,
		service.db.Executor(),
		models.SaveCustomCaddyRouteData{
...
func (service CaddyRouteService) validateCustomRoute(
	ctx context.Context,
	id uuid.UUID,
	input CustomCaddyRouteInput,
) (models.CustomCaddyRouteEntity, error) {
	...
	externalCount, err := service.db.Executor().NewSelect().TableExpr("custom_caddy_routes").
		...
		Where("removed_at IS NULL").Where("id <> ?", id).Count(ctx)
	if err != nil {
	...
	if externalCount+existingRouteCount > 0 {
		return models.CustomCaddyRouteEntity{}, domainError(
			"externalId",
			"unique",
			"Caddy route identifier is already in use",
		)
	}
	resourceEndpoints, err := models.ResourceEndpoint.ManagedActive(
		ctx,
		service.db.Executor(),
		uuid.Nil,
	)
	...
	hostnameConflicts, err := models.ResourceEndpoint.CaddyHostnameConflicts(
		ctx,
		service.db.Executor(),
		entity.Hostname,
		uuid.Nil,
	...
	customHostnameCount, err := service.db.Executor().NewSelect().TableExpr("custom_caddy_routes").
		Where("lower(hostname) = ?", entity.Hostname).
		Where("removed_at IS NULL").Where("id <> ?", id).Count(ctx)
	if err != nil {
		return models.CustomCaddyRouteEntity{}, err
	}
	if hostnameConflicts+customHostnameCount > 0 {
		return models.CustomCaddyRouteEntity{}, domainError(
			"hostname",
			"unique",
			"an active Caddy route already uses this hostname",
		)
	}
	return entity, nil
}

func (service CaddyRouteService) reconcileCustom(
	ctx context.Context,
	route models.CustomCaddyRouteEntity,
) error {
	...
	err := service.caddy.ApplyRoute(ctx, caddyclients.Route{
		...
		Backends: []caddyclients.Backend{
			{
				Dial:   net.JoinHostPort(route.OriginAddress, fmt.Sprint(route.OriginPort)),
				Weight: 100,
			},
		},
		HealthPath:                route.HealthPath,
		DisableActiveHealthChecks: route.HealthPath == "",
		UpstreamTLS:               protocol == "https",
	})
	now := time.Now().UTC()
	state, lastError, appliedAt := models.CaddyRouteApplied, sql.NullString{}, sql.NullTime{
		Time:  now,
		Valid: true,
	}
	if err != nil {
		state, lastError, appliedAt = models.CaddyRouteFailed, sql.NullString{
			String: err.Error(),
			Valid:  true,
		}, route.AppliedAt
	}
	_, updateErr := models.CustomCaddyRoute.Update(
		ctx,
		service.db.Executor(),
		models.SaveCustomCaddyRouteData{
...
func (service CaddyRouteService) DestroyCustom(ctx context.Context, id uuid.UUID) error {
	...
	now := time.Now().UTC()
	_, err = models.CustomCaddyRoute.Update(
		ctx,
		service.db.Executor(),
		models.SaveCustomCaddyRouteData{
			ID:             route.ID,
			...
			OriginProtocol: route.OriginProtocol,
			OriginTLSMode:  route.OriginTLSMode,
			HealthPath:     route.HealthPath,
			State:          models.CaddyRouteRemoved,
			AppliedAt:      route.AppliedAt,
			ObservedAt:     sql.NullTime{Time: now, Valid: true},
			RemovedAt:      sql.NullTime{Time: now, Valid: true},
		},
	)
	return err
}
