...
func (service *ReleaseDeployment) OrchestrateTx(ctx context.Context, tx bun.Tx, release models.ReleaseEntity, change models.ChangeEntity, revision models.EnvironmentStateRevisionEntity) (ReleaseOrchestrationResult, error) {
	if _, err := tx.ExecContext(ctx, "SELECT pg_advisory_xact_lock(hashtextextended(?, 0))", "release-orchestration:"+release.EnvironmentID.String()); err != nil {
	...
	if activeCommands != 0 || activeDeployments != 0 {
		return ReleaseOrchestrationResult{}, errors.New("Environment already has an active release operation")
	}
	...
	if len(targets) == 0 {
		return ReleaseOrchestrationResult{}, errors.Join(models.ErrDomainValidation, errors.New("Environment has no runtime Server targets"))
	}
	...
	if findErr == nil {
		...
		case "queued", "running":
			...
			return ReleaseOrchestrationResult{}, errors.New("Release command is already active for this Release")
		default:
			return ReleaseOrchestrationResult{}, errors.New("Release command requires manual recovery before deployment")
		}
	}
	...
	if releaseProcess, configured := state.ReleaseProcess(); configured {
		...
		execution, err := models.ReleaseCommandExecution.Create(ctx, tx, models.CreateReleaseCommandExecutionData{
			Configuration: configuration, ConfigurationDigest: digest, ReleaseID: release.ID,
			EnvironmentStateRevisionID: revision.ID, EnvironmentTargetID: targets[0].ID, ChangeID: change.ID,
		})
		if err != nil {
			return ReleaseOrchestrationResult{}, err
		}
...
func (service *ReleaseDeployment) FanOutSucceededReleaseCommand(ctx context.Context, executionID uuid.UUID, exitCode int32, externalID string) error {
	...
	revision, err := models.EnvironmentStateRevision.Find(ctx, tx, execution.EnvironmentStateRevisionID)
	if err != nil {
		return err
	}
	state, err := models.ParseEnvironmentDesiredState(revision.State)
	...
	now := time.Now().UTC()
	if _, err := tx.NewUpdate().TableExpr("release_command_executions").Set("status = 'succeeded'").Set("external_id = ?", externalID).Set("exit_code = ?", exitCode).Set("finished_at = ?", now).Set("error = NULL").Set("updated_at = ?", now).Where("id = ?", execution.ID).Where("status = 'running'").Exec(ctx); err != nil {
		return err
	}
	return tx.Commit()
}

func (service *ReleaseDeployment) fanOutTx(ctx context.Context, tx bun.Tx, release models.ReleaseEntity, change models.ChangeEntity, revision models.EnvironmentStateRevisionEntity, state models.EnvironmentDesiredState, targets []models.EnvironmentTargetEntity) (ReleaseOrchestrationResult, error) {
	existing, err := tx.NewSelect().Model((*models.DeploymentEntity)(nil)).Where("change_id = ?", change.ID).Count(ctx)
	if err != nil {
		return ReleaseOrchestrationResult{}, err
	}
	if existing > 0 {
		var first models.DeploymentEntity
		err := tx.NewSelect().Model(&first).Where("change_id = ?", change.ID).OrderExpr("created_at, id").Limit(1).Scan(ctx)
		return ReleaseOrchestrationResult{Deployment: first}, err
	}
	now := time.Now().UTC()
	if _, err := tx.NewUpdate().TableExpr("environment_target_states").Set("desired_revision_id = ?", revision.ID).Set("state = 'pending'").Set("updated_at = ?", now).Where("environment_target_id IN (SELECT id FROM environment_targets WHERE environment_id = ? AND detached_at IS NULL)", release.EnvironmentID).Exec(ctx); err != nil {
		return ReleaseOrchestrationResult{}, err
	}
	var first models.DeploymentEntity
	for targetIndex, target := range targets {
		deployment, err := service.QueueTargetTx(ctx, tx, change.ID, release.ID, target.ID, 1, json.RawMessage(`{"type":"blue_green","web_replicas":1}`), state.Processes, now)
		if err != nil {
			return ReleaseOrchestrationResult{}, err
		}
		if targetIndex == 0 {
...
func (service *ReleaseDeployment) QueueTargetTx(ctx context.Context, tx bun.Tx, changeID, releaseID, targetID uuid.UUID, attempt int32, strategy json.RawMessage, processes []models.EnvironmentProcessState, now time.Time) (models.DeploymentEntity, error) {
	...
	deployment, err := models.Deployment.Create(ctx, tx, models.CreateDeploymentData{
		Attempt: attempt, Strategy: strategy, ProcessConfiguration: processSnapshot,
	...
	for _, process := range processes {
		...
		for replica := int32(1); replica <= process.Replicas; replica++ {
			...
			if _, err := models.Instance.Create(ctx, tx, models.CreateInstanceData{
				ExternalID: "pending:" + deployment.ID.String() + ":" + replicaKey, Slot: "candidate",
				ProcessName: process.Name, ProcessKind: process.Kind, ReplicaKey: replicaKey, State: "candidate",
