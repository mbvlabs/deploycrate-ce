...
func (service *NodeEnrollment) List(ctx context.Context) ([]NodeEnrollmentDetail, error) {
	...
	for _, server := range servers {
		enrollment, err := models.NodeEnrollment.LatestForServer(ctx, service.db.Executor(), server.ID)
		if err != nil {
			return nil, err
		}
		credential, err := models.ServerSSHCredential.FindForServer(ctx, service.db.Executor(), server.ID)
		if err != nil {
			return nil, err
		}
		items = append(items, NodeEnrollmentDetail{Server: server, Enrollment: enrollment, Credential: credential})
	}
	return items, nil
}

func (service *NodeEnrollment) Get(ctx context.Context, id uuid.UUID) (NodeEnrollmentDetail, error) {
...
func (service *NodeEnrollment) Create(ctx context.Context, input CreateNodeInput) (NodeEnrollmentDetail, error) {
	...
	if err := validateSSHPrivateKey([]byte(input.PrivateKey), []byte(input.Passphrase)); err != nil {
		return NodeEnrollmentDetail{}, errors.Join(models.ErrDomainValidation, validation.ValidationErrors{{Field: "privateKey", Code: "invalid", Message: err.Error()}})
	}
	input.Capabilities.Telemetry = true
	capabilities, err := input.Capabilities.JSON()
	if err != nil {
		return NodeEnrollmentDetail{}, errors.Join(models.ErrDomainValidation, validation.ValidationErrors{{Field: "capabilities", Code: "invalid", Message: err.Error()}})
	}
	...
	server, err := models.Server.Create(ctx, tx, models.CreateServerData{
		Name: input.Name, Slug: slug.Make(input.Name), Kind: "worker",
		Capabilities: capabilities,
		Ipv4Address:  ipv4Address, Ipv6Address: ipv6Address, IsConfigured: false, Address: input.Address,
	})
	if err != nil {
		return NodeEnrollmentDetail{}, err
	}
	privateKey, err := secretcrypto.EncryptForPurpose([]byte(input.PrivateKey), service.config.App.SessionEncryptionKey, "node-enrollment/"+server.ID.String()+"/private-key")
	if err != nil {
		return NodeEnrollmentDetail{}, err
	}
	var passphrase []byte
	if input.Passphrase != "" {
		passphrase, err = secretcrypto.EncryptForPurpose([]byte(input.Passphrase), service.config.App.SessionEncryptionKey, "node-enrollment/"+server.ID.String()+"/private-key-passphrase")
		if err != nil {
			return NodeEnrollmentDetail{}, err
		}
	}
	credential, err := models.ServerSSHCredential.Create(ctx, tx, models.CreateServerSSHCredentialData{
...
func (service *NodeEnrollment) Confirm(ctx context.Context, id uuid.UUID, fingerprint string) error {
	...
	if strings.TrimSpace(fingerprint) != detail.Enrollment.HostFingerprint {
		return errors.Join(models.ErrDomainValidation, validation.ValidationErrors{{Field: "fingerprint", Code: "mismatch", Message: "fingerprint does not match the discovered SSH host key"}})
	}
	...
	if err := models.NodeEnrollment.Transition(ctx, tx, id, models.NodeEnrollmentQueued, "queued", nil); err != nil {
		return err
	}
	inserted, err := service.queue.InsertTx(ctx, tx.Tx, jobs.NodeEnrollmentArgs{EnrollmentID: id}, jobs.NodeEnrollmentInsertOpts(id))
	if err != nil {
		return err
	}
...
func (service *NodeEnrollment) Retry(ctx context.Context, id uuid.UUID) error {
	...
	if detail.Enrollment.State != models.NodeEnrollmentFailed || !detail.Credential.HostKeyConfirmedAt.Valid || len(detail.Credential.EncPrivateKey) == 0 {
...
func (service *NodeEnrollment) Execute(ctx context.Context, id uuid.UUID) (returnErr error) {
	defer func() {
		if returnErr != nil {
			_ = models.NodeEnrollment.Transition(context.WithoutCancel(ctx), service.db.Executor(), id, models.NodeEnrollmentFailed, "failed", returnErr)
		}
	}()
	detail, err := service.Get(ctx, id)
	if err != nil {
	...
	if err := models.NodeEnrollment.Transition(ctx, service.db.Executor(), id, models.NodeEnrollmentInstalling, "remote_install", nil); err != nil {
		return err
	}
	peer, peerErr := models.WireGuardPeer.FindActiveForServer(ctx, service.db.Executor(), detail.Server.ID)
	...
	if errors.Is(peerErr, sql.ErrNoRows) {
		...
		publicAddress := net.JoinHostPort(detail.Server.Address, strconv.Itoa(int(detail.Credential.Port)))
...
${dc_sudo} /usr/local/bin/bootstrap node install --manifest-stdin`, nodeInstallerURL)
		...
		installedHostKey, _, _, remainder, parseErr := ssh.ParseAuthorizedKey([]byte(result.SSHHostPublicKey))
		...
		detail.Server.DistributionVersion = sql.NullString{String: result.DistributionVersion, Valid: true}
		...
		if _, err := service.updateServer(ctx, service.db.Executor(), detail.Server, false); err != nil {
			return err
		}
		peer, err = models.WireGuardPeer.Create(ctx, service.db.Executor(), models.CreateWireGuardPeerData{
			PublicKey: result.WireGuardPublicKey, PrivateAddress: detail.Enrollment.AllocatedAddress,
			Endpoint:   sql.NullString{String: net.JoinHostPort(detail.Server.Address, "51820"), Valid: true},
			ListenPort: 51820, ActivatedAt: time.Now().UTC(), ServerID: detail.Server.ID,
		})
		if err != nil {
			return err
		}
	...
	if _, err := hostcommand.Run(ctx, "node-telemetry-target", detail.Server.ID.String(), detail.Enrollment.AllocatedAddress); err != nil {
		return err
	}
	if err := models.NodeEnrollment.Transition(ctx, service.db.Executor(), id, models.NodeEnrollmentVerifying, "ssh_ca_verification", nil); err != nil {
		return err
	}
	...
	privateAddress := net.JoinHostPort(detail.Enrollment.AllocatedAddress, strconv.Itoa(int(detail.Credential.Port)))
	verifyCommand := fmt.Sprintf("sudo -n true && curl -fsS http://%s:9100/metrics >/dev/null && curl -fsS http://%s:9101/healthz >/dev/null && curl -fsS http://127.0.0.1:13133/ >/dev/null", detail.Enrollment.AllocatedAddress, detail.Enrollment.AllocatedAddress)
	if _, err := service.ssh.RunWithCertificate(ctx, privateAddress, "admin", detail.Credential.KnownHostKey, certificate.PrivateKey, certificate.Certificate, verifyCommand, nil); err != nil {
	...
	if err := models.ServerSSHCredential.CompleteTrustTransition(ctx, tx, detail.Server.ID); err != nil {
		return err
	}
	if err := models.NodeEnrollment.Transition(ctx, tx, id, models.NodeEnrollmentReady, "complete", nil); err != nil {
		return err
	}
	return tx.Commit()
}
...
func (service *NodeEnrollment) updateServer(ctx context.Context, db storage.Executor, server models.ServerEntity, configured bool) (models.ServerEntity, error) {
	return models.Server.Update(ctx, db, models.UpdateServerData{
		ID: server.ID, ArchivedAt: server.ArchivedAt, Name: server.Name, Slug: server.Slug,
		Kind: server.Kind, Capabilities: server.Capabilities, OperatingSystem: server.OperatingSystem,
		Distribution: server.Distribution, DistributionVersion: server.DistributionVersion,
		Architecture: server.Architecture, PackageManager: server.PackageManager, InitSystem: server.InitSystem,
		Ipv4Address: server.Ipv4Address, Ipv6Address: server.Ipv6Address,
		IsConfigured: configured, Address: server.Address,
	})
}

func (service *NodeEnrollment) manifest(ctx context.Context, detail NodeEnrollmentDetail) (nodeinstall.Manifest, error) {
	var control models.WireGuardPeerEntity
	if err := service.db.Executor().NewSelect().Model(&control).Where("private_address = ?", WireGuardPrivateAddress).
...
func (service *NodeEnrollment) decryptBootstrapCredential(detail NodeEnrollmentDetail) ([]byte, []byte, error) {
	privateKey, err := secretcrypto.DecryptForPurpose(detail.Credential.EncPrivateKey, service.config.App.SessionEncryptionKey, "node-enrollment/"+detail.Server.ID.String()+"/private-key")
	if err != nil {
		return nil, nil, err
	}
	var passphrase []byte
	if len(detail.Credential.EncPrivateKeyPassphrase) > 0 {
		passphrase, err = secretcrypto.DecryptForPurpose(detail.Credential.EncPrivateKeyPassphrase, service.config.App.SessionEncryptionKey, "node-enrollment/"+detail.Server.ID.String()+"/private-key-passphrase")
	}
...
func validateCreateNodeInput(input CreateNodeInput) error {
	...
	if input.Name == "" {
		errs = append(errs, validation.Error{Field: "name", Code: "required", Message: "name is required"})
	}
	if input.Address == "" || strings.ContainsAny(input.Address, "\r\n\x00 /\\") {
		errs = append(errs, validation.Error{Field: "address", Code: "invalid", Message: "address is invalid"})
	}
	if input.Port < 1 || input.Port > 65535 {
		errs = append(errs, validation.Error{Field: "port", Code: "invalid", Message: "port must be between 1 and 65535"})
	}
	if input.Username == "" {
		errs = append(errs, validation.Error{Field: "username", Code: "required", Message: "username is required"})
	}
	if strings.TrimSpace(input.PrivateKey) == "" {
		errs = append(errs, validation.Error{Field: "privateKey", Code: "required", Message: "private key is required"})
	}
	return errs
}
