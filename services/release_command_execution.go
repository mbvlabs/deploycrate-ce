...
func NewReleaseCommandExecution(db storage.Pool, queue storage.InsertQueue, secrets *EnvironmentSecrets, builds *BuildExecution, workloads *WorkloadExecution, deployments *DeploymentExecution, releases *ReleaseDeployment) *ReleaseCommandExecution {
	return &ReleaseCommandExecution{db: db, queue: queue, secrets: secrets, builds: builds, workloads: workloads, deployments: deployments, releases: releases}
}
...
func newReleaseCommandLogger(ctx context.Context, db storage.Pool, execution models.ReleaseCommandExecutionEntity, redactions []string) (*releaseCommandLogger, error) {
	sequence, err := models.ReleaseCommandLog.NextSequence(ctx, db.Executor(), execution.ID, execution.Attempt)
	if err != nil {
		return nil, err
	}
	...
	sort.Slice(encodedRedactions, func(left, right int) bool { return len(encodedRedactions[left]) > len(encodedRedactions[right]) })
	return &releaseCommandLogger{
		ctx: executionContext{db: db, executionID: execution.ID, attempt: execution.Attempt}, sequence: sequence,
		redactions: encodedRedactions, pending: make(map[string][]byte), maxRedaction: maxRedaction,
	}, nil
}
...
func (logger *releaseCommandLogger) redactPrefix(value []byte, limit int) ([]byte, int) {
	...
	for consumed < limit {
		...
		for _, redaction := range logger.redactions {
			if len(value)-consumed >= len(redaction) && bytes.Equal(value[consumed:consumed+len(redaction)], redaction) {
...
func (logger *releaseCommandLogger) persist(stream string, value []byte) error {
	...
	for strings.TrimSpace(message) != "" {
		...
		if _, err := models.ReleaseCommandLog.Create(context.Background(), logger.ctx.db.Executor(), logger.ctx.executionID, logger.ctx.attempt, logger.sequence, stream, chunk); err != nil {
			return err
		}
		logger.sequence++
...
func (logger *releaseCommandLogger) recordTruncation() error {
	...
	_, err := models.ReleaseCommandLog.Create(context.Background(), logger.ctx.db.Executor(), logger.ctx.executionID, logger.ctx.attempt, logger.sequence, "system", "Release command output reached the 2 MiB persistence limit. Later output is omitted.")
	logger.sequence++
	return err
}
...
func (service *ReleaseCommandExecution) Execute(ctx context.Context, executionID uuid.UUID) error {
	...
	if execution.Status == "succeeded" || execution.Status == "failed" || execution.Status == "ambiguous" {
		return nil
	}
	configuration, err := models.ParseReleaseCommandConfigurationSnapshot(execution.Configuration, execution.ConfigurationDigest)
	if err != nil {
	...
	if err != nil {
		return service.fail(context.WithoutCancel(ctx), execution, false, nil, fmt.Errorf("resolve exact Environment revision secrets: %w", err))
	}
	...
	if err != nil {
		return service.fail(context.WithoutCancel(ctx), execution, false, nil, fmt.Errorf("initialize release command logs: %w", err))
	}
	if err := logger.system(ctx, fmt.Sprintf("Starting release command attempt %d", execution.Attempt)); err != nil {
		return service.fail(context.WithoutCancel(ctx), execution, false, logger, fmt.Errorf("persist release command start: %w", err))
	}
	networkName, err := service.workloads.ReconcileNetwork(ctx, target.ServerID, scope.Environment.ID)
	if err != nil {
		return service.fail(context.WithoutCancel(ctx), execution, false, logger, err)
	}
	environment, attachments, err := service.deployments.composeEnvironment(ctx, scope, resolved, false)
	if err != nil {
		return service.fail(context.WithoutCancel(ctx), execution, false, logger, err)
	}
	for _, attachment := range attachments {
		if err := service.workloads.ConnectResourceContainer(ctx, target.ServerID, scope.Environment.ID, attachment); err != nil {
			return service.fail(context.WithoutCancel(ctx), execution, false, logger, err)
		}
	}
	...
	commandContext, cancel := context.WithTimeout(ctx, time.Duration(configuration.TimeoutSeconds)*time.Second)
	defer cancel()
	result, runErr := service.workloads.RunOneOff(commandContext, target.ServerID, containerclient.OneOffRunSpec{
		ApplicationID: scope.ApplicationID, EnvironmentID: scope.Environment.ID, TargetID: target.ID,
		ReleaseID: release.ID, ReleaseCommandExecutionID: execution.ID, Attempt: execution.Attempt,
		ContainerName:  "dc-release-" + release.ID.String() + "-" + fmt.Sprint(execution.Attempt),
		ImageReference: release.ArtifactReference, NetworkName: networkName, Environment: environment,
		...
		Created: func(identifier string) error {
			_, err := service.db.Executor().NewUpdate().TableExpr("release_command_executions").Set("external_id = ?", identifier).Set("updated_at = ?", time.Now().UTC()).Where("id = ?", execution.ID).Where("status = 'running'").Exec(context.WithoutCancel(ctx))
			return err
		},
	}, credentials)
	logErr := logger.flushOutput()
	if result.ContainerCreated && result.State.Exists && !result.State.Running && result.State.Status == "exited" && result.State.ExitCode == 0 {
		_ = logger.system(ctx, "Release command completed successfully")
		if err := service.releases.FanOutSucceededReleaseCommand(context.WithoutCancel(ctx), execution.ID, result.State.ExitCode, result.State.ID); err != nil {
			return service.fail(context.WithoutCancel(ctx), execution, true, logger, fmt.Errorf("commit release command outcome and deployment fan-out: %w", err))
		}
		_ = service.workloads.RemoveOneOff(context.WithoutCancel(ctx), target.ServerID, result.State.ID)
		return nil
	}
	if runErr != nil {
		var exitCode *int32
		if result.ContainerCreated && result.State.Exists && !result.State.Running && result.State.Status == "exited" {
			value := result.State.ExitCode
			exitCode = &value
		}
		return service.fail(context.WithoutCancel(ctx), execution, result.ContainerCreated, logger, errors.Join(runErrWithExit(runErr, exitCode), logErr))
	}
	if logErr != nil {
		return service.fail(context.WithoutCancel(ctx), execution, result.ContainerCreated, logger, fmt.Errorf("persist release command output: %w", logErr))
	}
	if result.State.ExitCode != 0 {
		exitCode := result.State.ExitCode
		return service.fail(context.WithoutCancel(ctx), execution, true, logger, runErrWithExit(errors.New("release command exited unsuccessfully"), &exitCode))
	}
	return service.fail(context.WithoutCancel(ctx), execution, result.ContainerCreated, logger, errors.New("release command outcome is unavailable"))
}
...
func (service *ReleaseCommandExecution) claim(ctx context.Context, id uuid.UUID) (models.ReleaseCommandExecutionEntity, error) {
...
func (service *ReleaseCommandExecution) loadScope(ctx context.Context, execution models.ReleaseCommandExecutionEntity) (models.ReleaseEntity, models.EnvironmentStateRevisionEntity, models.EnvironmentTargetEntity, deploymentScope, error) {
	...
	revision, err := models.EnvironmentStateRevision.Find(ctx, service.db.Executor(), execution.EnvironmentStateRevisionID)
	if err != nil || revision.EnvironmentID != release.EnvironmentID {
		return release, revision, models.EnvironmentTargetEntity{}, deploymentScope{}, errors.New("release command revision is unavailable or mismatched")
	}
	...
	if err != nil || target.EnvironmentID != release.EnvironmentID || target.DetachedAt.Valid {
		return release, revision, target, deploymentScope{}, errors.New("release command target is unavailable or detached")
	}
	...
	if err != nil || environment.ArchivedAt.Valid {
		return release, revision, target, deploymentScope{}, errors.New("release command Environment is unavailable")
	}
	state, err := models.ParseEnvironmentDesiredState(revision.State)
	if err != nil {
		return release, revision, target, deploymentScope{}, err
	}
	scope := deploymentScope{Release: release, Target: target, Environment: environment, Revision: revision, State: state, ApplicationID: environment.ApplicationID}
	if release.RegistryResourceID != nil && release.RegistryCredentialID != nil && release.RegistryEndpoint.Valid {
		...
		return release, revision, target, deploymentScope{}, errors.New("Release registry snapshot is unavailable")
	}
	return release, revision, target, scope, nil
}

func (service *ReleaseCommandExecution) fail(ctx context.Context, execution models.ReleaseCommandExecutionEntity, created bool, logger *releaseCommandLogger, operationErr error) error {
	...
	now := time.Now().UTC()
	if err := models.ReleaseCommandExecution.MarkFailed(ctx, service.db.Executor(), execution.ID, status, exitCode, operationErr, now); err != nil {
		return err
	}
...
func (service *ReleaseCommandExecution) Retry(ctx context.Context, applicationID, environmentID, executionID, actorID uuid.UUID, replacementTargetID *uuid.UUID) error {
	tx, err := service.db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	...
	if target.ID != execution.EnvironmentTargetID {
		if _, err := tx.NewUpdate().TableExpr("release_command_executions").Set("environment_target_id = ?", target.ID).Where("id = ?", execution.ID).Exec(ctx); err != nil {
			return err
		}
	}
	now := time.Now().UTC()
	if err := models.ReleaseCommandExecution.ResetForRetry(ctx, tx, execution.ID, &actorID, now); err != nil {
		return err
	}
...
func (service *ReleaseCommandExecution) Logs(ctx context.Context, applicationID, environmentID, executionID uuid.UUID) ([]models.ReleaseCommandLogEntity, error) {
